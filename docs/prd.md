...
- [11. 库存管理规则](#11-库存管理规则)
  - [11.1 库存预留（锁定）与可用量](#111-库存预留锁定与可用量)
...
- [17. 页面字段与操作明细](#17-页面字段与操作明细)
  ...
  - [17.3 我的物资](#173-我的物资)
    - [17.3.1 领取记录（mine_pending_pickup）](#1731-领取记录-mine_pending_pickup)
    - [17.3.2 借还记录（mine_pending_return）](#1732-借还记录-mine_pending_return)
    - [17.3.3 归还确认（mine_return_confirm）](#1733-归还确认-mine_return_confirm)
  - [17.4 物资申请](#174-物资申请)
  ...
  - [17.10 基础配置](#1710-基础配置)
    ...
    - [17.10.3 分类管理（config_category）](#17103-分类管理-config_category)
    - [17.10.3.1 分类层级与编码](#171031-分类层级与编码)
    - [17.10.3.2 数据模型](#171032-数据模型-material_category)
    - [17.10.3.3 三类大类属性对照](#171033-三类大类属性对照)
    - [17.10.3.4 主页面与子类列表](#171034-主页面与子类列表)
    - [17.10.3.5 固定资产](#171035-固定资产大类与子类)
    - [17.10.3.6 类资产](#171036-类资产大类与子类)
    - [17.10.3.7 耗材](#171037-耗材大类与子类)
    - [17.10.3.8 属性继承、覆盖与约束](#171038-属性继承覆盖与约束)
    - [17.10.3.9 功能权限与页面映射](#171039-功能权限与页面映射)
...
| 待处置 | 作废单**已执行**后、实物尚未完成处置/丢弃/堆放的队列；存放于报废暂存区，由「物资处置」模块处理 |
| 报废暂存区 | 仓库内逻辑货位（如 `主仓库/报废暂存区`），承接已作废待实物处置的物资 |
...
| 物资处置 | 物资处置 | `warehouse_disposal_list.html` | `disposal_order` | 待执行、处置中、已完成 |
...
| 库存事务一致性 | 100% | 任意出库/入库/归还确认后，`inventory_location.qty ≥ 0` 且 `available + locked + borrowed + pending_confirm_qty ≤ qty`（§11.1.1） | 自动化对账脚本每日跑批，异常为 0 |
...
| 维度 | 说明 |
|------|------|
| 功能介绍 | 领用人视角的 **领用领取台账**：展示本人领用申请审核通过后的物资明细及出库进度，覆盖待领取与已领取全历史（非仅待办列表）。 |
| 前置条件 | 领用申请 `status = 审核通过`。 |
| 数据权限 | 普通用户仅查看 `applicant_id = 当前用户` 的记录；系统管理员、仓库管理员可查看全部。 |
| 页面跳转 | 「查看」→ 领用记录详情弹窗（`mine_requisition_record`）。 |

**业务定位**

| 对比项 | 领取记录（领用人） | 领用申请（编制） | 物资出库（仓管） |
|--------|-------------------|------------------|------------------|
| 角色 | 领用人 | 领用人 | 仓库管理员 |
| 关注点 | 我申请的物资领到了没有 | 发起与审批进度 | 执行出库、扣库存 |
| 数据粒度 | **领用明细行**（一物资一行） | 领用单头 + 明细 | 出库单（可拆多货位） |
| 可否出库 | 否（只读跟踪） | 否 | 是 |
...
sequenceDiagram
    participant U as 领用人
    participant R as 领用申请
    participant P as 领取记录
    participant O as 物资出库
    participant RT as 借还记录

    U->>R: 提交并审核通过
    R->>P: 按明细行生成 pickup_line<br/>status=待领取
    R->>O: 生成待出库任务（锁库已完成）
    O->>P: 出库完成回填 outbound_no/qty/date
    P->>P: 行 status→已领取（该行出清）
    alt 物资需归还
        O->>RT: 创建 return_tracking<br/>status=待归还
    end
```

| 时点 | 系统动作 |
|------|----------|
| 领用 **审核通过** | 按 `requisition_line` 逐行写入 `requisition_pickup_line`，`pickup_status = 待领取`；同步生成仓管侧待出库单（§3.5.4、§11.1） |
| 仓管 **部分出库** | 回填该行 `outbound_qty`（累计）、`outbound_no`；若累计出库 &lt; 申请数量，行保持 **待领取** |
| 仓管 **该行出清** | 当 `outbound_qty ≥ apply_qty`（或固资按件出完），行 `pickup_status → 已领取` |
| 出库完成且 **需归还** | 另建 `return_tracking`（见 §3.2.2），**不替代**领取记录 |

**状态判定**

| pickup_status | 判定条件 | 列表 Tab |
|---------------|----------|----------|
| 待领取 | 尚无出库单，或 `SUM(outbound_line.qty) < requisition_line.qty` | 待领取 |
| 已领取 | 该行申请数量已全部出库完成 | 已领取 |

**物资类型差异**

| 类型 | 是否进入领取记录 | 行展示要点 |
|------|:----------------:|------------|
| 固定资产 | ✓ | 按领用明细行；出库后回填 `asset_code`（在出库单/详情中）；一物一件 |
| 类资产 | ✓ | 按数量；支持部分出库，领取记录反映累计出库量 |
| 耗材 | ✓ | 按数量；一般 **不需归还**，出库后仅停留在「已领取」，不产生借还记录 |

**边界规则**

- **一领用单多物资** → 领取记录 **多行**（每明细一行），共享同一领用单号。
- **审核驳回/关闭**且未出库 → 对应 pickup 行删除或标记无效，列表不再展示。
- **超发禁止**：出库量不得大于申请量；领取记录出库数量以实际出库为准。
- 领用人 **不可** 在领取记录发起出库或修改申请；仅查看与跟踪。

**Tab**：全部 / 待领取 / 已领取

**列表字段**：领用申请单号、物资编码/名称、规格、申请数量、出库单号、出库数量、出库日期等（字段清单 §17.3.1）。

#### 3.2.2 借还记录

| 维度 | 说明 |
|------|------|
| 功能介绍 | 领用人视角的 **借出与归还全生命周期台账**：仅包含「需归还」物资，从出库借出到归还确认完结（含超期、分批归还、作废）。 |
| 前置条件 | 出库完成；物资 `need_return = 需要`（分类/物资清单继承，§10）。 |
| 数据权限 | 普通用户仅 `borrower_id = 当前用户`；管理员、仓管可查看全部。 |
| 页面跳转 | 「查看」→ 归还单/借还详情（只读）；「确认」→ `mine_return_confirm`（仅待确认）。 |

**与领取记录的分工**

| 模块 | 适用范围 | 核心问题 |
|------|----------|----------|
| **领取记录** | 所有审核通过的领用明细 | 申请的数量领到了没有 |
| **借还记录** | 其中 `need_return = 需要` 的 subset | 借出的何时还、是否超期、是否已确认入库 |

耗材默认 **不进入** 借还记录；若个别耗材分类配置为需归还，则按类资产数量规则处理。

**数据产生**

| 触发 | 规则 |
|------|------|
| 出库完成 | 对每条需归还的出库结果创建 `return_tracking` |
| 固定资产 | **一物一条**：`return_key = requisition_no + asset_code`；`outbound_qty = 1` |
| 类资产 | 可按 **领用明细行** 一条跟踪，或按出库批次拆分；`pending_return_qty = outbound_qty − returned_qty` |
| 应还日期 `due_date` | `outbound_completed_at` 的日期 + **有效借用周期**（天）；周期取自物资清单 → 叶子分类 → 大类（§17.10.3.8） |

**应还日期计算**

```
due_date = DATE(outbound.completed_at) + borrow_days_effective
```

`borrow_days_effective`：优先物资实例配置，否则分类链有效值；未配置时 V1.0 默认 90 天（可配置项）。
...
| status | 含义 | 借用人可操作 | 仓管侧（§3.5.5） |
|--------|------|--------------|------------------|
| 待归还 | 已借出，未还完，未超期 | 查看 | 归还入库、作废 |
| 已延期 | `today > due_date` 且仍有 `pending_return_qty > 0` | 查看；工作台超期提醒 | 同待归还 |
| 部分归还 | 已还部分，`0 < returned_qty < outbound_qty` | 查看 | 继续归还 |
| 待确认 | 仓管已归还入库，待借用人确认 | **查看、确认** | 查看（不可代确认） |
| 已归还 | 借用人已确认，流程完结 | 查看 | 查看 |
| 已作废 | 仓管作废，库存不恢复 | 查看 | 查看 |

**超期规则（已延期）**

- **批处理**（每日）：`due_date < 当前日期` 且 `status ∈ {待归还, 部分归还}` 且 `pending_return_qty > 0` → 更新为 **已延期**。
- 已延期 **不阻断** 仓管办理归还；归还后仍走待确认 → 已归还。
- KPI「借还超期率」见 §1.5。

**分批归还（类资产/耗材需归还场景）**

1. 仓管在「物资归还」录入本次归还数量 &lt; `pending_return_qty`。
2. `returned_qty` 累加；若仍未还完 → `status = 部分归还`（列表 Tab 归入 **待归还**）。
3. 本批入库后 → 生成归还单，跟踪行进入 **待确认**（针对本批）；借用人确认后，若仍有待还数量 → 回到 **部分归还/待归还**，等待下一批。

**与物资归还模块关系**：借还记录为 **领用人只读 + 确认** 视角；实物归还、作废由仓库管理员在「物资归还」执行（§3.5.5）。领用人 **不在** 借还记录发起归还入库（与领取记录相同，执行权在仓管）。

**Tab**：全部 / 待归还 / 已延期 / 待确认 / 已归还（**部分归还** 归入待归还 Tab）

**列表字段**：领用单号、资产编码、物资名称、状态、出库信息、应还日期、待还数量等（§17.3.2）。
...
| 维度 | 说明 |
|------|------|
| 功能介绍 | 发起物资领用请求，审核通过后**锁定库存**并自动生成出库待办。 |
| 前置条件 | 可选关联已审核通过的物资计划；编制时参考**可用库存**（§11.1.2）。 |
...
| 时点 | 行为 |
|------|------|
| 保存草稿 / 提交审核 | **不锁库**；校验申请数量 ≤ 当前可用量 |
| **审核通过** | 按明细**锁定**库存（`available ↓`，`locked ↑`），生成待出库单与领取记录 |
| 驳回 / 撤回 / 关闭 | **释放**未出库部分的锁定 |
| 出库执行 | 自 `locked` 转实扣 `qty`（§3.5.4、§2.13） |
...
| 前置条件 | 领用申请审核通过（库存**已锁定**）；出库数量 ≤ 该行剩余锁定量。 |
| 数据权限 | 仓库管理员操作。 |
| 页面跳转 | 「出库」→ 按类型分固定资产/类资产/耗材出库页。 |

**状态**：待出库 → 已出库（支持部分出库）

**库存规则**：审核通过时已预留（锁定）；出库时 `locked` 转实扣 `qty`；分配策略见 **§11.2**；可用量公式见 **§11.1.2**。
...
| 维度 | 说明 |
|------|------|
| 功能介绍 | 管理不可继续使用物资的报废流程：待报废池汇聚**待报废**物资，经作废申请执行后更新台账与审计；**已执行**作废的实物进入**待处置**队列，由 §3.5.8 物资处置完成处置/丢弃/堆放。 |
...
flowchart TB
    ...
    subgraph ledger["泳道 · 系统/台账"]
        ...
        L2 --> L3["实物入报废暂存区<br/>状态=已作废·待处置"]
    end

    subgraph disposal["泳道 · 物资处置（§3.5.8）"]
        direction LR
        D1["待处置队列"] --> D2["发起处置单"] --> D3["现场执行"] --> D4["已完成"]
    end
...
**操作矩阵**：待执行 — 查看、执行；已执行 — 查看（实物若未处置，见待处置队列）

#### 3.5.8 物资处置 [V1.0]

| 维度 | 说明 |
|------|------|
| 功能介绍 | 对**已执行作废**的实物进行处置：处置（转让）、丢弃或临时堆放；核销台账由 §3.5.7 完成，本节负责实物离场或暂存管理。 |
| 前置条件 | 作废单 status=**已执行**；物资位于报废暂存区，状态为「已作废·待处置」。 |
...
|------|-------------------|----------------|
| 目的 | 台账核销、审计留痕 | 实物处置/丢弃/堆放 |
| 触发 | 待报废池 / 在库报废 | 作废**已执行**后自动入队 |
| 单据 | `ZF` 作废单 | `CZ` 处置单 |
| 库存 | 扣减 qty 或 asset→已作废 | 不再改台账 qty；处置（转让）可记处置收入 |

**处置方式与字段**：

| 方式 | 适用场景 | 关键字段 |
|------|----------|----------|
| **处置** | 有残值可回收、转让离场 | 买方/受让方、处置金额、收款方式 |
| **丢弃** | 无价值或法规要求销毁 | 丢弃地点、见证人、现场照片 |
| **堆放** | 暂存待清运 | 堆放位置、预计清运日期、保管责任人 |
...
flowchart TB
    ...
    subgraph sys["泳道 · 系统"]
        direction LR
        S1["生成 CZ 单号"] --> S2["处置：记处置收入<br/>丢弃/堆放：记处置流水"]
        S2 --> S3["从待处置队列移除"]
    end
...
维护物资大类（固定资产/类资产/耗材）与物资子类层级；不同类型大类承载的业务属性不同。**子类默认继承父级（大类→一级子类→二级子类）全部可继承属性，并可在表单中显式修改覆盖**；未覆盖项继续向上解析生效值。完整规则见 **§17.10.3.8**。

**子类继承与可覆盖属性（概要）**：

| 属性 | 说明 | 固定资产/类资产 | 耗材 | 子类可否修改 |
|------|------|-----------------|------|:------------:|
| 计量单位 | 默认继承父级，叶子可改 | ✓ | ✓ | ✓ |
| 是否归还 | 继承父级，子类可覆盖 | ✓ | 默认否 | ✓（受 §17.10.3.8 约束） |
| 是否需要盘点 | 继承父级，子类可覆盖 | ✓ | — | ✓ |
| 是否需要使用年限 | 继承父级，子类可覆盖 | ✓ | — | ✓ |
| 借用周期（天） | 二级子类可填；≤ 父级有效值 | ✓ | — | ✓ |
| 库存下限/上限/安全库存（开关） | 类/耗：继承大类开关，子类可覆盖 | 类资产 | 耗材 | ✓ |
| 库存下限/上限/安全库存（数值） | 子类可填默认值；区间不得宽于父级 | 类资产 | 耗材 | ✓ |
| 损耗标准（开关） | 耗材大类可配，子类可覆盖 | — | ✓ | ✓ |
...
**可用量计算**：`available = qty − locked − borrowed − pending_confirm_qty`（类资产/耗材）；固定资产按资产状态判定，详见 **§11.1.2**。
...
领用、出库、归还等业务围绕 **在库总量（`qty`）**、**锁定量（`locked`）**、**借出量（`borrowed`）**、**待确认量（`pending_confirm_qty`）** 与 **可用量（`available`）** 协同运作。货位层（`inventory_location`）为数量真相来源，汇总层（`inventory_summary`）由货位层聚合（§9.2）。

**核心原则**：

- **保存/提交**领用申请：校验申请数量 ≤ 当前可用量，**不锁库**。
- **审核通过**：事务内强校验并**锁定**库存（`available ↓`，`locked ↑`）；生成待出库单。
- **出库完成**：自锁定转实扣（`locked ↓`，`qty ↓`）；需归还物资 `borrowed ↑`。
- **驳回/撤回/关闭**：释放未出库部分的锁定。
- **部分出库**：已出库部分扣减；剩余保持锁定直至出库完毕或单据关闭。

与 §2.9 出库状态机、§2.13 领用出库时序图一致。

#### 11.1.1 数量字段定义

| 字段 | 货位层 | 汇总层 | 含义 |
|------|:------:|:------:|------|
| `qty` | ✓ | ✓（Σ） | **在库总量**：账面在库数量；固资在库时为 1，借出/待报废等态下货位 `qty` 为 0 |
| `locked` | ✓ | ✓（Σ） | **锁定量**：领用审核通过、尚未出库的预留数量 |
| `borrowed` | ✓ | ✓（Σ） | **借出量**：已出库、借用未归还的数量（归还跟踪汇总） |
| `pending_confirm_qty` | ✓ | ✓（Σ） | **待确认量**：归还已入库、借用人尚未确认的数量（§11.7） |
| `available` | ✓ | ✓（计算） | **可用量**：可被新领用选中、可参与出库分配的数量 |

**对账恒等式**（类资产/耗材，单货位行或汇总行）：

```
available + locked + borrowed + pending_confirm_qty ≤ qty
```

各项均 ≥ 0；与 §1.5 KPI、AC-20 一致。

#### 11.1.2 可用数量计算公式

**（一）类资产 / 耗材 — 货位层**

对 `inventory_location` 同一货位行（唯一键见 §9.2.2）：

```
available = qty − locked − borrowed − pending_confirm_qty
```

运算在**同一货位**内闭合；跨货位合并见汇总层。

**（二）类资产 / 耗材 — 汇总层（`inventory_summary`）**

按 `material_code` 聚合（页面若限定仓库，则仅聚合同一 `warehouse_id` 下货位行）：

```
qty_total               = Σ 货位层.qty
locked_total            = Σ 货位层.locked
borrowed_total          = Σ 货位层.borrowed
pending_confirm_total   = Σ 货位层.pending_confirm_qty

available = qty_total − locked_total − borrowed_total − pending_confirm_total
```

**领用选物资 / 编制表单**默认展示汇总层 `available`；若业务指定领用仓库，则展示该仓库下货位层 `available` 之和。

**（三）固定资产 — 按件（一物一码）**

不使用上述数量公式，以**资产实例**为粒度判定是否可选：

| 条件 | 说明 |
|------|------|
| `asset_status = 在库` | 必要条件 |
| 未被其他领用单锁定 | `inventory_lock` 有效记录或货位 `locked` 标记 |
| 盘点冻结（§13.1） | 冻结范围内禁止新增锁定与出库；V1.0 已在途待出库可执行完毕 |

领用数量恒为 **1**；审核通过锁定 `asset_code`；出库后 `asset_status → 借出`（需归还）或等效出库态，货位 `qty` 扣减。

**（四）盘点冻结**

库场盘点勾选「冻结出入库」时，范围内货位**禁止新增锁定与出库**（接口与按钮拦截）；不单独增加 `frozen_qty` 字段，不改变 `qty` 账面值。V1.0 对已锁定待出库单**默认允许执行完毕**。

**（五）入库**

采购验收入库：`qty ↑`，`available` 按公式同步增加；**不经过** `locked`。
...
|----------|----------|------|
| 领用保存（草稿） | 无 | 不写锁库记录 |
| 领用提交审核 | 无 | 校验申请数量 ≤ `available`（可提示，不占用） |
| **领用审核通过** | **锁库**：`locked += 申请数量`，`available` 按 §11.1.2 减少 | **强校验** `申请数量 ≤ available`；失败则审批不落库 |
| 驳回 / 撤回 / 关闭 | **释放**：`locked -= 剩余未出库量`，`available` 回升 | 已部分出库的仅释放剩余 |
| **出库执行完成** | **扣减**：`locked -= 本次出库量`，`qty -= 本次出库量`；需归还则 `borrowed += 本次出库量` | 校验 `本次出库量 ≤ 该行剩余 locked` |
| 出库取消（实物未发） | **释放**对应锁定量 | — |
| 部分出库 | 已出部分扣减 `qty`/`locked`；余量保持锁定 | 出库单 `status = 部分出库` |

固定资产：审核通过写入 `inventory_lock(requisition_id, asset_code)`；出库消锁并更新 `asset_status`。

#### 11.1.4 领用单状态 × 库存动作

| 领用单状态 | `locked` | `qty` | 说明 |
|------------|----------|-------|------|
| 草稿 / 审核中 | 不变 | 不变 | 可编辑；审核中不可改明细数量 |
| 审核通过（待出库） | +申请量 | 不变 | 待出库、可部分出库 |
| 部分出库 | 随出库递减 | 已出部分已扣 | 继续出库 |
| 已出库 | 0（该行） | 已扣减 | 进入归还跟踪（若需归还） |
| 审核失败 / 已关闭 | 释放至 0 | 不变 | 可编辑重提 |

#### 11.1.5 锁库明细（`inventory_lock`）

| 字段 | 说明 |
|------|------|
| `lock_id` | 主键 |
| `source_type` | `requisition`（扩展：调拨等） |
| `source_no` | 领用单号 `LY…` |
| `source_line_id` | 明细行 ID |
| `material_code` | 物资编码 |
| `asset_code` | 固资必填 |
| `warehouse_id` / 货位键 | 类资产/耗材：审核时可锁仓库级；出库按 §11.2 FIFO 落货位 |
| `lock_qty` | 锁定数量 |
| `released_qty` | 已释放（含转出库扣减） |
| `status` | 有效 / 已释放 |

V1.0 类资产/耗材可先增汇总层 `locked`，出库时再落货位明细；固资审核通过时必须锁定到 `asset_code`。

#### 11.1.6 校验摘要

| 校验点 | 规则 | 失败处理 |
|--------|------|----------|
| 添加物资 / 编制 | `申请数量 ≤ available` | 表单阻断 |
| **审核通过** | 同上，事务内强校验 | 审批失败，提示库存不足 |
| 出库 | `出库数量 ≤ 该行剩余 locked`；禁止 `qty < 0` | 超发阻断 |
| 并发 | 货位 `version` 乐观锁（§11.6） | 提示刷新后重试 |
...
- **固定资产/类资产**：禁止负库存
- **耗材**：V1.0 禁止负库存；出库数量不得大于**该行剩余锁定量**（`locked`）
...
| 在库/待报废作废执行 | 扣减货位 qty 或 asset→已作废；实物转入报废暂存区，**待处置**（见 §3.5.8） |
| 物资处置-处置 | 台账不再变动；记处置收入与 disposal 审计流水 |
| 物资处置-丢弃/堆放 | 台账不再变动；记 disposal 审计；堆放保留保管责任至清运 |
| 盘亏调整 | 调整库存减少 qty，需审批 |
...
| 可用 | 可领用/可出库数量 | `inventory_summary.available` | `qty − locked − borrowed − pending_confirm_qty`（§11.1.2） | 默认列 |
| 锁定 | 预留数量 | `inventory_summary.locked` | 领用审核通过未出库等 | 默认列 |
...
> 本章字段清单与 §3.2 业务规则配套：§3.2 描述领取/借还 **为何产生、如何流转**；本节描述 **页面字段、筛选与操作权限**。

#### 17.3.1 领取记录（mine_pending_pickup）

**页面说明**：领用人领取台账；原型 `mine_pending_pickup.html`。数据源视图 `requisition_pickup_line`（领用明细行 × 出库回填）。

##### 17.3.1.1 业务规则摘要

| 规则项 | 说明 |
|--------|------|
| 行粒度 | **一领用明细一行**（`requisition_id + line_id` 唯一）；同一领用单多物资占多行 |
| 生成时机 | 领用审核通过时批量插入，`pickup_status = 待领取` |
| 待领取 | `outbound_no` 为空，或累计 `outbound_qty < apply_qty` |
| 已领取 | 该行 `outbound_qty ≥ apply_qty`（固资为对应 `asset_code` 均已出库） |
| 部分出库 | 列表展示已累计 `outbound_qty` / `outbound_date`；状态仍为待领取直至出清 |
| 与借还 | 出库完成且需归还时另写 `return_tracking`；领取记录行可保持已领取，借还走 §17.3.2 |
| 数据范围 | 默认 `requisition.applicant_id = 当前用户`；管理员/仓管可查全部 |

**数据模型（`requisition_pickup_line`）**

| 字段 | 说明 |
|------|------|
| `pickup_line_id` | 主键 |
| `requisition_no` / `line_id` | 关联领用单与明细 |
| `material_code` | 物资编码 |
| `apply_qty` | 申请数量 |
| `outbound_no` | 最近/汇总出库单号（未出库为空） |
| `outbound_qty` | 累计已出库数量 |
| `outbound_completed_at` | 末次出库完成时间 |
| `pickup_status` | 待领取 / 已领取 |
| `applicant_id` | 申请人（权限过滤） |
...
| 出库数量 | 已出库数 | `outbound_line.qty` 累计 | 部分出库为已出累计 | 默认列 |
| 出库日期 | 出库完成日 | `outbound.completed_at` | 末次出库日 | 默认列 |

**筛选与搜索**

| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| Tab | 全部/待领取/已领取 | `tab` | 待领取：`pickup_status=待领取` 或出库未出清；已领取：行已出清 |
...
**页面说明**：借出归还全生命周期（领用人视角）；原型 `mine_pending_return.html`。数据源 `return_tracking`。

##### 17.3.2.1 业务规则摘要

| 规则项 | 说明 |
|--------|------|
| 准入条件 | 出库完成 + `need_return = 需要`；耗材默认不产生 |
| 跟踪粒度 | 固资：**一资产一条**（`asset_code` 必填）；类资产：按明细/批次，`asset_code` 可为 — |
| `pending_return_qty` | `outbound_qty − returned_qty`（已确认归还计入 `returned_qty`） |
| `due_date` | 出库完成日 + 有效 `borrow_days`（§3.2.2） |
| 超期 | 日批：`due_date` 已过且仍有待还 → `status = 已延期` |
| 部分归还 | `0 < returned_qty < outbound_qty` 时为部分归还；Tab 映射 **待归还** |
| 待确认 | 仓管归还入库后；借用人须在 `mine_return_confirm` 确认（§3.2.3、§17.3.3） |
| 已作废 | 仓管作废；跳过待确认；库存不恢复（§11.5） |
| 领用人操作 | **待归还/已延期/部分归还**：仅查看；**待确认**：查看 + 确认；**已归还/已作废**：仅查看 |
...
**数据模型（`return_tracking`）**

| 字段 | 说明 |
|------|------|
| `return_key` | 主键；固资建议 `requisition_no + asset_code` |
| `requisition_no` | 来源领用单 |
| `outbound_no` | 出库单号 |
| `asset_code` | 固定资产必填 |
| `material_code` | 物资编码 |
| `borrower_id` | 借用人（权限过滤） |
| `outbound_qty` | 借出数量 |
| `returned_qty` | 已归还数量（含已确认批次） |
| `pending_return_qty` | 计算：待还数量 |
| `due_date` | 应还日期 |
| `status` | 待归还/已延期/部分归还/待确认/已归还/已作废 |
| `last_return_order_no` | 最近归还单号（待确认/已归还时有值） |
...
| 状态 | 归还状态 | `return_tracking.status` | 见 §3.2.2；部分归还 Tab→待归还 | Tab 列（col 9） |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 查看 | `wms:mine:return:view` | 领用人/仓管/管理员 | 全部 | 归还/借还详情（只读） |
| 确认归还 | `wms:mine:return:confirm` | 领用人 | **仅待确认** | `mine_return_confirm` |

> 领用人 **无**「发起归还」操作；实物归还由仓管在 `warehouse_return_list` 执行（§16.5、§17.6.7）。

#### 17.3.3 归还确认（mine_return_confirm）

**页面说明**：借还记录「确认」入口；借用人核对仓管归还入库信息并确认。原型 `mine_return_confirm.html`。业务规则 §3.2.3；弹窗字段 §17.13.3.2。

| 规则项 | 说明 |
|--------|------|
| 前置状态 | `return_tracking.status = 待确认` |
| 操作人 | 必须 `borrower_id = 当前用户` |
| 确认后 | `status → 已归还`；若仍有 `pending_return_qty > 0` 则回到 **部分归还/待归还**（类资产分批场景） |
| 库存 | 确认后释放 `pending_confirm_qty`，`available` 恢复（§11.7） |
| 超时 | 7 日未确认：工作台提醒，**不**自动确认 |
...
| 领用明细 | 物资行 | 添加物资 | 是 | ≥1 行；申请数量 ≤ 可用库存（§11.1.2） | 审核通过时锁库 |
| 说明 | 备注 | 手工 | 否 | ≤500 字 | — |
| 凭证 | 附件 | 上传 | 否 | 格式限制 | — |
| 申请人 | 申请人 | 组织 | 是 | — | 默认当前用户 |
| 申请部门 | 部门 | 组织 | 是 | — | 默认主部门 |
...
| 申请人 | 申请人 | — | — | 默认列 |
| 申请部门 | 部门 | — | — | 默认列 |
| 状态 | 草稿/审核中/待执行/已执行 | `status` | Tab col 7 | Badge |
| 申请时间 | 时间 | — | — | 默认列 |
...
| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| Tab | 全部/草稿/审核中/待执行/已执行 | `tab` | — |
| 下拉 | 来源类型 | `filters.source` | 全部/在库报废/归还灭失/待报废转报废/维修报废 |
...
| 执行 | `wms:warehouse:scrap:execute` | 仓管 | 待执行 | `warehouse_scrap_execute.html` |
...
| 来源 | 来源类型 | `source_type` | 在库/归还损坏等 | 默认列 |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 发起作废 | `wms:warehouse:scrap:create` | 仓管 | 池内待处置 | `warehouse_scrap_form.html?poolKey=` |
...
| 处置方式 | 处置/丢弃/堆放 | `method` | 下拉 col 1 | Badge |
| 来源作废单 | ZF 单号 | `scrap_order.no` | 关联已执行作废单 | 默认列 |
| 物资摘要 | 摘要 | 明细 | — | 默认列 |
| 数量 | 处置数量 | SUM | 带单位 | 默认列 |
| 处置金额 | 金额 | `amount` | 非处置方式显示 — | 默认列 |
| 经办人 | 经办人 | — | — | 默认列 |
| 状态 | 待执行/处置中/已完成 | `status` | Tab col 7 | Badge |
...
| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| Tab | 全部/待执行/处置中/已完成 | `tab` | — |
| 下拉 | 处置方式 | `filters.method` | 全部/处置/丢弃/堆放 |
...
| 执行 | `wms:warehouse:disposal:execute` | 仓管 | 待执行/处置中 | `warehouse_disposal_execute.html` |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 发起处置 | `wms:warehouse:disposal:create` | 仓管 | 池内待处置 | `warehouse_disposal_form.html?poolKey=` |

#### 17.6.15 处置表单（warehouse_disposal_form / `*_sell_form` / `*_discard_form` / `*_stockpile_form`）
...
| 处置方式 | 处置/丢弃/堆放 | 选择/池带入 | 是 | 枚举 | 决定下方字段区 |
| 来源作废单 | ZF | 系统/池 | 是 | 只读；来自待处置时不可改 | — |
| 买方/受让方 | 处置 | 手工 | 处置必填 | 非空 | 提交 |
| 处置金额 | 处置 | 手工 | 处置必填 | >0 | 提交 |
| 收款方式 | 处置 | 下拉 | 处置必填 | 银行转账/现金/抵账 | 提交 |
| 丢弃地点 | 丢弃 | 手工 | 丢弃必填 | 非空 | 提交 |
| 见证人 | 丢弃 | 手工 | 丢弃必填 | ≥1 人 | 提交 |
| 现场照片 | 丢弃 | 上传 | 否 | 图片格式 | — |
| 堆放位置 | 堆放 | 下拉 | 堆放必填 | 场外堆放区等 | 提交 |
| 预计清运日期 | 堆放 | 日期 | 堆放必填 | ≥今日 | 提交 |
| 保管责任人 | 堆放 | 下拉 | 堆放必填 | — | 提交 |
| 处置物资明细 | 行 | 池带入/只读 | 是 | ≥1；来自池时不可换编码 | 提交 |
| 说明 | 备注 | 手工 | 否 | ≤500 字 | — |
| 经办人/部门 | 组织 | 下拉 | 是 | — | 提交 |
...
| 页面 | 说明 |
|------|------|
| `warehouse_disposal_form.html` | 新建处置单；可切换至处置/丢弃/堆放子页 |
| `warehouse_disposal_sell_form.html` | 处置方式预设（转让离场） |
| `warehouse_disposal_discard_form.html` | 丢弃方式预设 |
| `warehouse_disposal_stockpile_form.html` | 堆放方式预设 |
...
| 方式扩展信息 | 买方金额/丢弃地点/堆放位置等 | 按 `method` | — | 只读展示 |
| 处置明细表 | 物资/数量 | `disposal_line` | — | 只读 |

**提交逻辑**：`POST /api/v1/disposal-orders/{disposalKey}/execute`；状态→已完成；从 `disposal_pool` 移除对应行；处置（转让）写入处置收入记录；写 `txn_type=disposal` 审计与 `audit_log`。
...
**页面说明**：维护物资分类主数据，支撑物资清单、验收标准、盘点范围与全链路业务规则（§3.7.2、§10）。布局为 **左侧物资大类树 + 右侧子类表格**；资产类（固定资产、类资产）为系统预置大类，耗材类大类可由管理员新增。原型：`config_category.html`。

**权限**：`wms:config:category:*`；仅系统管理员可维护。

#### 17.10.3.1 分类层级与编码

```mermaid
flowchart TB
  subgraph asset_group["资产类（系统分组）"]
    M1["固定资产 ZC-GD<br/>系统预置大类"]
    M2["类资产 LA-ZC<br/>系统预置大类"]
    M1 --> S1["一级子类 ZC-GD-001"]
    M1 --> S2["一级子类 ZC-GD-002"]
    S1 --> L1["二级子类 ZC-GD-001001<br/>叶子节点"]
    M2 --> S3["一级子类 LA-ZC-001"]
    S3 --> L2["二级子类 LA-ZC-001001"]
  end
  subgraph consumable_group["耗材类（可扩展分组）"]
    M3["办公耗材 HC-BG<br/>可新增大类"]
    M4["生产耗材 / 劳保耗材…"]
    M3 --> S4["一级子类 HC-BG-001"]
    S4 --> L3["二级子类 HC-BG-001002"]
  end
```

| 层级 | 说明 | 编码规则（示例） | 可维护性 |
|------|------|------------------|----------|
| 分组 | 资产类 / 耗材类 | — | 固定，仅 UI 分组 |
| **物资大类** | 固定资产、类资产，或耗材下的「办公耗材」等 | 固资 `ZC-GD`；类资产 `LA-ZC`；耗材 `HC-{简码}` | 固资/类资产**预置不可删**；耗材大类可**新增/编辑/删除**（未引用时） |
| **一级子类** | 大类下第一层分类 | `{大类编码}-{三位序号}`，如 `ZC-GD-001` | 可增删改（未引用时） |
| **二级子类** | 一级子类下叶子分类 | `{一级编码}{三位序号}`，如 `ZC-GD-001001` | 可增删改（未引用时） |

**叶子节点规则**：物资清单（`material_catalog`）必须绑定 **叶子分类**（无下级子类的节点）；一级子类若存在二级子类，则一级节点不可被物资直接引用。

#### 17.10.3.2 数据模型（`material_category`）

| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `category_id` | string | ✓ | 主键 |
| `parent_id` | string | — | 父节点；大类父级为分组虚拟节点 |
| `level` | enum | ✓ | `major` / `sub_l1` / `sub_l2` |
| `material_type` | enum | ✓ | `fixed` / `like` / `consumable` |
| `code` | string | ✓ | 分类编码，同级唯一 |
| `name` | string | ✓ | 分类名称 |
| `unit` | string | 条件 | 主计量单位；**二级子类必填**，一级子类可为空（列表展示 `—`） |
| `need_return` | enum | 条件 | `需要` / `不需要`；固资/类资产大类与子类；耗材大类 |
| `need_inventory` | boolean | 条件 | 是否需要盘点；固资/类资产 |
| `need_service_life` | boolean | 条件 | 是否需要使用年限；固资/类资产 |
| `borrow_days` | int | 条件 | 借用周期（天）；**仅二级子类**（固资/类资产）；出库后计算应还日 |
| `require_min_stock` | boolean | 条件 | 是否启用「库存下限」字段（类资产大类/子类；耗材大类/子类） |
| `require_max_stock` | boolean | 条件 | 是否启用「库存上限」 |
| `require_safe_stock` | boolean | 条件 | 是否启用「安全库存」 |
| `require_loss_standard` | boolean | 条件 | 是否启用「损耗标准」；**仅耗材大类** |
| `min_stock` / `max_stock` / `safe_stock` | decimal | 条件 | 分类级默认阈值（当对应 `require_*`=是 且子类/物资未覆盖时生效） |
| `loss_standard` | decimal | 条件 | 损耗标准默认值（耗材，可选） |
| `remark` | string | — | 备注 |
| `enabled` | boolean | ✓ | 启用/停用 |
| `ref_count` | int | — | 被物资清单引用次数（>0 不可删） |
| `*_source` | enum | — | 各业务字段附带：`inherited`（继承父级）/ `override`（本子类显式配置）；未配置时存 `inherited` |

> **存储约定**：子类保存时，用户**未改动**的字段记为 `inherited`（库中业务值为空，读取时向上解析）；用户**显式选择/输入**的字段记为 `override` 并持久化。编辑表单打开时，继承项展示父级**有效值**并标注「继承自：{父级名称}」。

#### 17.10.3.3 三类大类属性对照

大类决定该类型物资的 **业务属性开关** 与默认规则；右侧主区顶部 **大类摘要条**（`categoryDetailHeader`）只读展示当前选中大类的已配置属性。

| 属性 | 含义 | 固定资产大类 | 类资产大类 | 耗材大类 |
|------|------|:------------:|:----------:|:--------:|
| 分类编码 | 系统或手工 | `ZC-GD`（预置） | `LA-ZC`（预置） | `HC-*`（新增时系统生成） |
| 分类名称 | 展示名 | 固定资产 | 类资产 | 如办公耗材 |
| **归还** | 出库后是否进入归还跟踪 | ✓ 需要/不需要 | ✓ | ✓（默认不需要） |
| **是否需要盘点** | 纳入库场/库外盘点范围（§3.8、§3.9） | ✓ 是/否 | ✓ | — |
| **是否需要使用年限** | 物资清单需填使用年限 | ✓ 是/否 | ✓ | — |
| **库存下限** | 是否在物资清单强制维护下限 | — | ✓ 需要/不需要 | ✓ 需要/不需要 |
| **库存上限** | 是否在物资清单强制维护上限 | — | ✓ 需要/不需要 | ✓ 需要/不需要 |
| **安全库存** | 是否在物资清单强制维护安全库存 | — | —（子类/物资填数值） | ✓ 需要/不需要 |
| **损耗标准** | 耗材损耗基准（扩展） | — | — | ✓ 需要/不需要（可选） |

> **说明**：大类上的「库存下限/上限/安全库存」为 **属性开关**（需要/不需要），表示下游物资清单是否必须填写对应数值；具体阈值在 **子类默认值** 或 **物资清单** 上维护（§17.10.6）。
...
| 节点 | 交互 | 目标 |
|------|------|------|
| 固定资产 / 类资产 | 单击切换右侧面板 | 子类表 + 大类摘要 |
| 耗材大类（办公/生产/劳保…） | 单击切换；悬停显示编辑/删除 | `config_category_major_consumable_form` |
| 树底「+」 | 新增耗材大类 | `config_category_major_consumable_form` |
...
|------|------|
| 搜索 | 关键字：`分类编码`、`分类名称` |
| 「+ 新增」 | 按当前大类类型打开对应子类表单（见 §17.10.3.9） |
| 大类摘要条 | 展示当前大类已启用的业务属性（只读） |
| 子类表格 | 支持一级展开/折叠、二级缩进展示 |
...
| 层级 | 操作项 | 目标页面 |
|------|--------|----------|
| 一级子类 | 添加子类、查看、编辑、删除 | `config_category_sub_child_form` / `*_view` / `*_form` |
| 二级子类 | 查看、编辑、删除 | `config_category_sub_*_view` / `*_form` |

#### 17.10.3.5 固定资产大类与子类

**查看固定资产大类**（`config_category_major_fixed`）

| 字段名 | 含义 | 数据来源 | 必填 | 校验/说明 |
|--------|------|----------|:----:|-----------|
| 分类编码 | `ZC-GD` | 系统预置 | — | 只读 |
| 分类名称 | 固定资产 | 系统预置 | — | 只读 |
| 归还 | 默认需归还 | `need_return` | ✓ | 需要/不需要 |
| 是否需要盘点 | 盘点纳入 | `need_inventory` | ✓ | 是/否 |
| 是否需要使用年限 | 清单填年限 | `need_service_life` | ✓ | 是/否 |

**新增/编辑一级子类**（`config_category_sub_fixed`）

| 字段名 | 含义 | 数据来源 | 必填 | 校验/提交逻辑 |
|--------|------|----------|:----:|---------------|
| 分类编码 | 子类编码 | 系统生成 | — | 只读；保存时按规则生成 |
| 分类名称 | 名称 | 手工 | ✓ | 1–50 字；同级不重名 |
| 计量单位 | 默认单位 | `unit` 主数据 | — | 一级可为空；建议选常用单位 |
| 归还 | 是否需归还 | 继承大类，**可覆盖** | ✓ | 枚举；覆盖时须满足 §17.10.3.8.3 |
| 是否需要盘点 | 盘点规则 | 继承大类，**可覆盖** | ✓ | 父级=是时子级不可改为否 |
| 是否需要使用年限 | 年限规则 | 继承大类，**可覆盖** | ✓ | 父级=是时子级不可改为否 |
| 备注 | 备注 | 手工 | — | ≤200 字 |
| 继承预览表 | 大类有效值 | 只读 | — | 展示父级当前**有效配置**（非仅默认值） |

**新增/编辑二级子类**（`config_category_sub_child_form`，`type=fixed`）

在一级子类字段基础上：

| 字段名 | 含义 | 必填 | 校验/提交逻辑 |
|--------|------|:----:|---------------|
| 父级分类 | 所属一级子类 | ✓ | 必选；不可跨大类 |
| 计量单位 | 叶子主单位 | ✓ | 默认继承一级；**可覆盖**为其他启用单位 |
| 归还 / 盘点 / 使用年限 | 业务规则 | ✓ | 默认继承一级有效值；**可覆盖**（§17.10.3.8.3） |
| 借用周期（天） | 默认借用天数 | — | 默认继承；**可覆盖**为 ≤ 父级有效值的天数 |

**查看子类**（`config_category_sub_fixed_view` / `config_category_detail`）：展示编码、名称、计量单位、归还、盘点、使用年限；二级另展示借用周期。

#### 17.10.3.6 类资产大类与子类

**查看类资产大类**（`config_category_major_like`）

在固定资产大类字段基础上，**增加**：

| 字段名 | 含义 | 必填 | 说明 |
|--------|------|:----:|------|
| 库存下限 | 属性开关 | ✓ | 需要/不需要；为是时物资清单须填下限 |
| 库存上限 | 属性开关 | ✓ | 需要/不需要 |

**新增/编辑子类**（`config_category_sub_like` / `config_category_sub_child_form`，`type=like`）

| 差异点 | 说明 |
|--------|------|
| 列表列 | 与固定资产子类表相同（盘点、使用年限） |
| 表单字段 | 归还、盘点、使用年限、借用周期均 **继承上级有效值，子类可覆盖** |
| 库存开关 | 大类已启用的下限/上限开关，一级/二级子类可 **继承或覆盖**；不可将父级「需要」改为「不需要」 |
| 库存数值默认 | 子类可填默认阈值，须落在 §17.10.3.8.3 区间内 |
| 物资清单 | 继承叶子子类有效值；物资可再次覆盖（§17.10.6） |
| 库存预警 | 以物资**有效**阈值参与预警（§3.1.1） |

类资产在业务上按 **物资编码 + 货位** 管理数量，分类层不直接存 `asset_code`，但继承的「归还」「借用周期」给出库后归还跟踪提供默认值（§10）。

#### 17.10.3.7 耗材大类与子类

**新增/编辑耗材大类**（`config_category_major_consumable_form`）

| 字段名 | 含义 | 数据来源 | 必填 | 校验/提交逻辑 |
|--------|------|----------|:----:|---------------|
| 分类编码 | `HC-*` | 系统生成 | — | 只读 |
| 分类名称 | 如办公耗材 | 手工 | ✓ | 1–50 字 |
| 归还 | 是否需归还 | 单选 | ✓ | 默认**不需要**；特殊可消耗品可设为需要 |
| 库存下限 | 属性开关 | 单选 | ✓ | 需要/不需要 |
| 库存上限 | 属性开关 | 单选 | ✓ | 需要/不需要 |
| 安全库存 | 属性开关 | 单选 | ✓ | 需要/不需要 |
| 损耗标准 | 属性开关 | 单选 | — | 需要/不需要；V1.0 可选 |

**新增/编辑耗材子类**（`config_category_sub_consumable` / 二级 `sub_child_form`，`type=consumable`）

| 字段名 | 含义 | 必填 | 校验/提交逻辑 |
|--------|------|:----:|---------------|
| 分类编码 / 名称 | 基础 | 名称 ✓ | 同固资子类 |
| 计量单位 | 主单位 | 二级 ✓ | 继承父级，**可覆盖** |
| 库存下限 / 上限 / 安全库存 | 开关与数值 | — | 默认继承大类有效配置；**可覆盖**开关与默认数值（§17.10.3.8.3） |
| 损耗标准 | 开关/数值 | — | 仅当大类启用时可继承；子类可覆盖 |
| 备注 | 备注 | — | ≤200 字 |

**无字段**：是否需要盘点、是否需要使用年限、借用周期（耗材默认不纳入资产年限与借用模型，§10）。

**子类列表列**：库存下限、库存上限、安全库存（展示子类已配置数值；一级汇总行可为 `—`）。

#### 17.10.3.8 属性继承、覆盖与约束

##### 17.10.3.8.1 继承链与生效值

每一级分类（大类 → 一级子类 → 二级子类）对可继承属性均支持 **继承（inherited）** 或 **覆盖（override）** 两种状态。子类在新增/编辑表单中**可以修改**任一可继承字段；保存后若与父级有效值不同，则记为本级覆盖。

```mermaid
flowchart LR
  M["物资大类<br/>有效值 = 自身配置"]
  L1["一级子类<br/>有效值 = 覆盖值 ?? 大类"]
  L2["二级子类（叶子）<br/>有效值 = 覆盖值 ?? 一级 ?? 大类"]
  MC["物资清单<br/>有效值 = 物资覆盖 ?? 叶子子类链"]
  M --> L1 --> L2 --> MC
```
...
```
effective(N, attr) =
  若 N.attr 为 override → N.attr_value
  否则若 N.parent 存在 → effective(N.parent, attr)
  否则 → 类型默认值
```

物资清单在叶子分类有效值基础上仍可覆盖（§17.10.6）；业务单据（领用、出库、归还）以 **物资实例有效值** 为准（§10）。

##### 17.10.3.8.2 可继承属性清单（按物资类型）

| 属性字段 | 固资大类 | 类资产大类 | 耗材大类 | 一级子类 | 二级子类 | 子类可修改 |
|----------|:--------:|:----------:|:--------:|:--------:|:--------:|:----------:|
| 计量单位 `unit` | — | — | — | 可覆盖 | 可覆盖（叶子必填） | ✓ |
| 归还 `need_return` | 配置 | 配置 | 配置 | 可覆盖 | 可覆盖 | ✓ |
| 是否需要盘点 `need_inventory` | 配置 | 配置 | — | 可覆盖 | 可覆盖 | ✓ |
| 是否需要使用年限 `need_service_life` | 配置 | 配置 | — | 可覆盖 | 可覆盖 | ✓ |
| 借用周期 `borrow_days` | — | — | — | 可覆盖（可选） | 可覆盖 | ✓ |
| 库存下限开关 `require_min_stock` | — | 配置 | 配置 | 可覆盖 | 可覆盖 | ✓ |
| 库存上限开关 `require_max_stock` | — | 配置 | 配置 | 可覆盖 | 可覆盖 | ✓ |
| 安全库存开关 `require_safe_stock` | — | — | 配置 | 可覆盖 | 可覆盖 | ✓ |
| 损耗标准开关 `require_loss_standard` | — | — | 配置 | 可覆盖 | 可覆盖 | ✓ |
| 库存下限/上限/安全库存 **数值** | — | 可配默认 | 可配默认 | 可覆盖 | 可覆盖 | ✓ |
| 损耗标准 **数值** | — | — | 可配默认 | 可覆盖 | 可覆盖 | ✓ |

「配置」= 该层级表单必显式维护；「可覆盖」= 默认继承父级有效值，子类保存时可改为 `override`。

##### 17.10.3.8.3 子类覆盖规则（可改，但不得违反父级约束）

子类**允许修改**继承项，但须满足下列 **单调性 / 区间约束**（父级更严则子级不能放宽）：

| 属性类型 | 父级有效值 | 子级覆盖允许范围 | 违反时提示 |
|----------|------------|------------------|------------|
| **开关·合规类** | | | |
| 是否需要盘点 | 是 | 仅可为 **是** | 「父级已要求盘点，子类不可关闭」 |
| 是否需要使用年限 | 是 | 仅可为 **是** | 「父级已要求使用年限，子类不可关闭」 |
| 归还 | 需要 | 仅可为 **需要** | 「父级要求归还，子类不可改为不需要」 |
| 库存下限/上限/安全库存开关 | 需要 | 仅可为 **需要** | 「父级已启用该字段，子类不可关闭」 |
| **开关·可放宽** | | | |
| 是否需要盘点 | 否 | 可为 **是** 或 **否** | — |
| 归还 | 不需要 | 可为 **需要** 或 **不需要** | — |
| 各库存开关 | 不需要 | 可为 **需要** 或 **不需要** | — |
| **数值·借用周期** | 父级 = P 天 | 子级为空（继承）或 **1 ≤ 子级 ≤ P** | 「借用周期不得大于父级（P 天）」 |
| **数值·库存阈值** | 父级下限 Lp、上限 Up、安全 Sp | 子级 Lc、Uc、Sc 须：**Lp ≤ Lc ≤ Sc ≤ Uc ≤ Up** | 「子类库存区间不得宽于父级」 |
| **计量单位** | 父级单位 U | 子级可选任意启用单位；**二级叶子必填** | 二级未填阻断保存 |

> 类资产无大类级「安全库存开关」时，安全库存数值在**物资清单**必填（§17.10.6）；子类若配置默认数值，仍须落在父级已启用的上下限开关所允许的区间内。

##### 17.10.3.8.4 表单交互（新增/编辑子类）

| 交互 | 规则 |
|------|------|
| 打开表单 | 各可继承字段预填 **父级有效值**；继承项旁展示灰色提示「继承自：{父级名称}」 |
| 用户修改字段 | 控件值变化后标记为 **待覆盖**；保存时写入 `override` |
| 恢复继承 | 提供「恢复继承」操作（或清空为继承态），保存后删除本级 `override`，重新使用父级有效值 |
| 继承预览表 | 一级固资/类资产子类表单底部只读表，展示**大类**当前有效配置（原型 `inheritTable`） |
| 父级变更联动 | 若父级日后调严规则，子级已 `override` 的宽松值在下次编辑保存时**重新校验**，不通过则阻断并提示调整 |
| 查看弹窗 | 展示 **有效值** + 标注来源（「本级配置」/「继承自 XXX」） |

**示例（类资产 · 电动工具 LA-ZC-001 → 电钻 LA-ZC-001001）**

| 字段 | 大类 LA-ZC 有效值 | 一级子类 LA-ZC-001 | 二级子类 LA-ZC-001001 | 叶子有效值（物资默认继承） |
|------|-------------------|--------------------|-----------------------|---------------------------|
| 归还 | 需要 | 继承 | 继承 | 需要 |
| 是否需要盘点 | 是 | 继承 | 继承（不可改为否） | 是 |
| 借用周期 | — | — | **override 60 天** | 60 天 |
| 库存下限开关 | 需要 | 继承 | 继承 | 需要 |
| 库存下限数值 | — | **override 5** | **override 8**（须 ≥5） | 8 |

##### 17.10.3.8.5 多级覆盖示例

```
办公耗材 HC-BG（大类）
  require_safe_stock = 需要
  safe_stock 默认 = 50
  └─ 办公用纸 HC-BG-001（一级，override safe_stock = 30）
       └─ 打印纸 A4 HC-BG-001002（二级，继承 safe_stock 数值 → 30）
            └─ 物资 HC-00089（清单 override safe_stock = 25，且 ≥ 下限、≤ 上限）
```

##### 17.10.3.8.6 删除、停用与下游

| 条件 | 允许操作 |
|------|----------|
| 未被物资清单引用 | 删除、编辑、停用 |
| 已被引用（`ref_count>0`） | **不可删除**；可编辑部分属性、可停用 |
| 停用分类 | 不可被选入新物资/计划/采购/领用；已存在单据不受影响 |

**下游影响**

| 模块 | 关联方式 |
|------|----------|
| 物资清单 | 必选叶子分类；动态列与必填项由 `material_type` + 分类属性开关决定 |
| 验收标准 | 按**叶子子类**维护一套标准（§17.10.7） |
| 库场盘点 | `need_inventory=是` 的类/固资纳入计划范围（§3.8） |
| 库外盘点 | 固资/类资产按分类筛选应盘范围（§3.9） |
| 领用/出库/归还 | 归还、借用周期来自分类 → 物资继承链（§10、§11.1） |

#### 17.10.3.9 功能权限与页面映射

| 操作项 | 权限码 | 可见角色 | 可见条件 | 目标 |
|--------|--------|----------|----------|------|
| 查看分类树/子类表 | `wms:config:category:view` | 管理员 | — | `config_category.html` |
| 新增耗材大类 | `wms:config:category:create` | 管理员 | — | `config_category_major_consumable_form` |
| 查看固资/类资产大类 | `wms:config:category:view` | 管理员 | — | `config_category_major_fixed` / `_like` |
| 新增/编辑子类 | `wms:config:category:edit` | 管理员 | 未引用或允许改 | `config_category_sub_*` / `sub_child_form` |
| 查看子类 | `wms:config:category:view` | 管理员 | — | `config_category_*_view` / `detail` |
| 删除子类/耗材大类 | `wms:config:category:delete` | 管理员 | `ref_count=0` | 确认后删除 |
| 停用 | `wms:config:category:edit` | 管理员 | — | 状态切换 |

**弹窗页面索引**

| 页面 | 类型 | 适用 |
|------|------|------|
| `config_category_major_fixed` | 查看 | 固定资产大类 |
| `config_category_major_like` | 查看 | 类资产大类 |
| `config_category_major_consumable_form` | 新增/编辑 | 耗材大类 |
| `config_category_sub_fixed` | 新增/编辑 | 固定资产一级子类 |
| `config_category_sub_like` | 新增/编辑 | 类资产一级子类 |
| `config_category_sub_consumable` | 新增/编辑 | 耗材一级子类 |
| `config_category_sub_child_form` | 新增/编辑 | 二级子类（按 URL/上下文区分 fixed/like/consumable） |
| `config_category_sub_*_view` / `config_category_detail` | 查看 | 各级子类只读 |

**接口（V1.0）**：`GET/POST/PUT/DELETE /api/v1/material-categories`；树形查询 `GET .../tree?type=`；删除前 `GET .../{id}/refs` 返回引用数。
...
| 可用数量 | 可出库量 | `inventory_summary.available` | `qty − locked − borrowed − pending_confirm_qty`（§11.1.2） | — | 无提交 |
| 锁定/预留 | 审核通过未出库 | `inventory_summary.locked` | 领用审核通过未出库等 | — | 无提交 |
...
**弹窗说明**：领取记录「查看」；只读展示领用申请头 + 明细 + 出库回填信息（若已出库）。尺寸 lg。参数：领用单 `no`、`back`。

**业务说明**：数据来自 `requisition` + `requisition_line` + 关联 `outbound`；不单独落库。出库未完成时出库区字段为空；部分出库展示已累计出库数量（§3.2.1）。
...
**弹窗说明**：借用人确认仓管已完成的归还入库；只读展示借出上下文 + 本批归还单 + 入库货位。尺寸 lg。参数：`returnKey`、`back`。

**业务说明**：仅 `status=待确认` 且当前用户为借用人时可提交；确认后驱动 `return_tracking` 状态与库存 `available` 释放（§3.2.3、§11.7）。
...
**提交逻辑**：`POST /api/v1/scrap-orders/{scrapKey}/execute`；状态=已执行；扣减货位 qty 或 asset→已作废；写 scrap 流水与 audit_log；**实物写入待处置队列**（`disposal_pool`），引导用户进入 §3.5.8。
...
| 归还/是否需要盘点/使用年限/借用周期 | 固资/类资产 | 分类配置 | 展示**有效值**及继承来源标注 | — | 无提交 |
| 库存下限/上限/安全库存/损耗标准 | 类资产/耗材 | 分类配置 | 开关+数值；标注继承/本级覆盖 | — | 无提交 |
| 计量单位 | 子类 | category | 叶子展示有效单位 | — | 无提交 |
...
| 在库总量 | 账面在库 | `inventory_summary.qty` 或仓库子集 Σ | — | 只读 |
| 已锁定 | 预留未出库 | `inventory_summary.locked` 或仓库子集 Σ | — | 只读 |
| 可用库存数量 | 可领用量 | 按 §11.1.2 计算 | — | 只读；`qty − locked − borrowed − pending_confirm_qty` |
| 申请领用数量 | 领用数 | 用户 | **是** | 固资固定 1 只读；类资产/耗材 ≤ 可用库存且 >0 |
| 计划领用日期 | 日期 | 用户 | 建议填 | date |
| 备注 | 行备注 | 用户 | 否 | ≤200 字 |
| 操作 | 删除 | — | — | 移除行 |

表底提示：**固定资产**按件领用数量固定 1；**类资产、耗材**须填写申请领用数量且不得超过**可用库存**（公式见 §11.1.2）。审核通过时按申请量锁库，提交/保存阶段不占用锁定量。
...
| 关键字 | 搜索 | 用户 | 编码/名称 | 否 | — |
| 勾选列 | 多选 | 用户 | 启用物资 | **是** | 至少 1 行 |
| 物资编码/名称/类型/单位/在库/已锁定/可用库存/仓库 | 列表列 | catalog+库存 | 可用量按 §11.1.2；仅启用物资 | — | 无提交 |
...
| V1.5.15 | 2026-07-02 | 补充领取记录/借还记录业务与逻辑：§3.2.1–3.2.2 链路、状态、类型差异；§17.3.1–17.3.3 数据模型与规则摘要；修正借用人不可发起归还 |
| V1.5.14 | 2026-07-02 | §17.10.3.8 扩充属性继承与覆盖：有效值解析、子类可修改规则、单调性约束、表单「继承/覆盖」交互与多级示例；§3.7.2、§17.10.3.2/5–7、§17.13.5.3 同步 |
| V1.5.13 | 2026-07-02 | 扩充 §17.10.3 分类管理：分类层级与编码、数据模型、三类大类属性对照、固资/类资产/耗材子类表单字段、继承校验与页面映射；§3.7.2 交叉引用 |
| V1.5.12 | 2026-07-02 | 领用库存三层模型写入 §11.1：扩充可用量计算公式（货位层/汇总层/固资按件）、锁库/释放/扣减时机与 `inventory_lock`；§3.3.2、§3.5.4、§9.2.1、§17.4.4、§17.13.10.3 对齐；新增 AC-27 |
...
| AC-03 | P0 | 领用 → 出库 → 领取记录 | 库存充足 | 领用审核 → 出库 → 查看领取记录 | 审核通过 `locked↑`；出库后 `qty↓`、`locked↓`；待领取→已领取；流水可查 | §3.3.2、§11.1 |
...
| AC-07b | P0 | 待处置 → 处置执行 | 作废已执行 | 待处置池发起处置（处置/丢弃/堆放）→ 执行 | 生成 CZ 单；队列清除；处置记收入；有 audit_log | §3.5.8 |
| AC-08 | P0 | 库存两层一致性 | 类资产多货位 | 分别入库 A/B 货位 → 查物资总览与仓库台账 | 总览按编码汇总；台账按编码+货位分行；数量一致 | §9.2 |
| AC-09 | P0 | 负库存/超发阻断 | 可用库存不足 | 两单并发领用同一物资 → 第二单审核或超量出库 | 审核或出库阻断；`available` 不为负 | §11.1.6、§11.4 |
...
| AC-20 | P2 | 库存对账脚本 | 有业务数据 | 跑批对账任务 | `qty≥0` 且 `available+locked+borrowed+pending_confirm_qty≤qty`；异常 0 | §1.5、§11.1.1 |
...
| AC-27 | P0 | 领用锁库与释放 | 类资产有库存 10 | 领用 6 审核通过 → 驳回另一单 / 撤回 | 通过后 `locked=6`、`available=4`；释放后 `locked` 回 0、`available` 恢复 | §11.1.3 |
