...
| 产品版本 | V1.0.0 |
| **版本更新时间** | 2026-08-06 |
| **版本迭代说明** | 基础业务交付；分类管理增加「领用周期」及超期报警（工作台 + 物资计划） |
| 部署区域 | 湖北黄冈武穴 |
| 目标平台 | PC Web |
| 交互原型 | [全局总入口](https://liaoliao66.github.io/wms/prototype/index.html) · [v1.0.0 原型门户](https://liaoliao66.github.io/wms/prototype/versions/v1.0.0/index.html) |
| 文档状态 | 本期交付版：以侧栏菜单为准（无盘点菜单）；验收审核、物资验收等见 §17 |
| 最后更新 | 2026-08-06 |
...
- [3. 功能模块总览](#3-功能模块总览)
  - [3.0 原型入口门户（v1.0.0）](#30-原型入口门户v100)
  - [3.1 物资台账](#31-物资台账)
  ...
  - [3.6 供应商管理](#36-供应商管理)
    - [3.6.1 入库审核](#361-入库审核)
    - [3.6.2 供应商列表](#362-供应商列表)
    - [3.6.3 供应商评价](#363-供应商评价)
  - [3.7 基础配置](#37-基础配置)
    - [3.7.1 计量单位](#371-计量单位)
    - [3.7.2 分类管理](#372-分类管理)
    - [3.7.3 验收标准](#373-验收标准)
...
- [17. 页面字段与操作明细](#17-页面字段与操作明细)
  ...
  - [17.3 我的物资](#173-我的物资)
    - [17.3.1 领用记录（mine_pending_pickup）](#1731-领用记录)
    - [17.3.2 归还记录（mine_pending_return）](#1732-归还记录)
  - [17.4 物资申请](#174-物资申请)
  ...
  - [17.9 供应商](#179-供应商)
    - [17.9.1 入库审核（supplier_admission_list）](#1791-入库审核)
    - [17.9.2 供应商列表（supplier_list）](#1792-供应商列表)
    - [17.9.3 供应商新增/编辑（supplier_form）](#1793-供应商新增编辑)
    - [17.9.4 供应商详情（supplier_detail）](#1794-供应商详情)
    - [17.9.5 供应商评价（supplier_eval_list）](#1795-供应商评价)
...
| 领用记录 | 领用申请审核通过后的领取明细视图（原「待领物资」菜单名） |
| 归还记录 | 需归还资产的借出与归还全生命周期视图（原「待还物资」菜单名） |
| 领用周期 | 分类配置（天）。以计划物资对应**采购入库完成日**为起点；`当前日期 − 入库完成日 > 领用周期` 且仍有未领用完额度时报警，提示计划编制异常。与「借用周期」（出库后应还期限）不同 |
| 借用周期 | 分类配置（天）。出库完成后计算应还日期；超时进入借还超期 |
...
|----------------|----------|----------|---------------|--------------|
| 领用记录 | 领用记录 | `mine_pending_pickup.html` | `requisition_pickup_line` | 待领取、已领取 |
| 归还记录 | 归还记录 | `mine_pending_return.html` | `return_tracking` | 待归还、已延期、部分归还、待确认、已归还、已作废 |
...
| 物资归还 | 物资归还 | `warehouse_return_list.html` | `return_order` | 同归还记录（仓管视角） |
...
**命名约定**：工作台指标、统计口径可使用业务短语（如「待还超期」「借还超期」）；用户可见菜单以本表为准。历史文档中的「待领物资」「领取记录」「待还物资」「借还记录」仅作迁移说明，不再作为菜单文案。
...
| 分类 | 范围定义 | v1.0.0 处理原则 | 优先级 |
|------|----------|------------------|:------:|
| 本版本包含 | 已在原型与侧栏落地的业务模块 | 纳入本 PRD 详细设计与页面清单 | P0 |
| 本版本不包含 | 未在原型落地或明确延期的业务能力 | 仅保留边界声明，不展开详细方案 | P1 |
| 待规划 | 规则允许但本版本未交付的终端/能力 | 在总入口与版本说明中标记「待规划」 | P2 |
| 平台公共能力 | 消息、审批、角色、权限、审计等平台底座 | 仅写承接边界，不单独设计页面/按钮/弹窗 | P0 |

**In Scope（本期必做 · 与原型对齐）**

| 域 | 包含能力 |
|----|----------|
| 门户与工作台 | 双端入口、PC 工作台统计卡片与异常物资提醒 |
...
| 供应商管理 | **入库审核**、供应商列表、评价、**供应商供货单** |
| 基础配置 | 单位、分类、验收标准、物资清单、仓库配置、使用地点、评价设置 |
| 流程配置 | 业务流程模板入口页（仅作为平台配置映射说明，不展开平台审批设计） |
| 移动端 | 资产确认、**库场盘点 APP**（6 页）、**库外盘点 APP**（11 页） |
| 通用规范 | 详情弹窗、树+列表布局、单据编号、库存两层模型、业务数据可见范围 |
...
| 项目 | 当前状态 | 说明 |
|------|----------|------|
| 企微 H5 原型 | 待规划 | 当前仅交付 PC Web + 独立 App |
| 供应商外部门户 | 待规划 | 本版本仍为内部代录与内部查看模式 |
| 复杂审批编排 | 待规划 | 走平台能力，不在本 PRD 详细展开 |

#### 1.6.2 平台公共能力承接说明

| 平台能力 | 本 PRD 处理方式 | 备注 |
|----------|-----------------|------|
| 消息中心 | 不设计独立页面，仅在业务结果中描述“由平台通知” | 不纳入页面清单 |
| 流程审批 | 不设计平台审批页面，仅保留业务触发节点与状态结果 | 不展开节点 UI |
| 角色管理 | 不设计角色增删改查页面 | 仅说明业务角色视角 |
| 权限配置 | 不设计权限配置台 | 仅保留业务数据可见范围原则 |
| 操作审计 | 不设计审计中心页面 | 仅描述需留痕的关键业务动作 |

### 1.7 本期研发交付清单

| 模块 | 优先级 | 关键页面/能力 | 字段与操作明细 |
|------|:------:|---------------|----------------|
| 工作台 | P0 | 统计卡片、异常物资提醒 | §17.1 |
| 物资台账 | P0 | 库存总览、仓库台账、出入库记录 | §17.2 |
| 我的物资 | P0 | 领用记录、归还记录、归还确认 | §17.3 |
| 物资申请 | P0 | 物资计划、领用申请 | §17.4 |
| 采购管理 | P0 | 待采、采购申请、物资采购 | §17.5 |
| 物资管理 | P0 | 验收、验收审核、入出库、归还、退货、作废、**处置** | §17.6 |
| 库场盘点 | — | 已迁至 **v1.2.0** | 见 v1.2.0 PRD |
| 库外盘点 | — | 已迁至 **v1.1.0** | 见 v1.1.0 PRD |
| 供应商管理 | P1 | 入库审核、供应商列表、评价、供货单 | §17.9 |
| 基础配置 | P0 | 单位、分类、清单、仓库、地点、评价 | §17.10 |
| 流程配置 | P2 | 平台流程模板映射说明页 | §17.11 |
| 移动端 | P1 | 资产确认、库场/库外移动盘点 | §17.12 |
| **弹窗** | P0 | 详情/确认/选择器/二维码 | **§17.13** |

> **说明**：登录鉴权、审批待办、消息通知、角色权限配置为平台公共能力，不作为本版本业务交付清单单列页面。
...
**一级概述**：按 `global_pipeline` 规范，原型采用**双层入口**——全局总入口汇总全部版本；各版本门户提供 PC / 独立 App 双卡片。

**场景归类**

| 场景 | 说明 | 优先级 |
|------|------|:------:|
| 正常流程 | 用户从 `prototype/index.html` 进入版本表格，再进入 `prototype/versions/v1.0.0/index.html` 选择 PC / App | P0 |
| 边界情况 | GitHub Pages 访问需带正确相对路径；版本门户需始终回链全局总入口 | P1 |
| 异常兜底 | 若版本页面未生成，则在全局入口表格中明确标注“待生成/待规划”，不展示失效入口 | P1 |
| 风险项 | 新增版本时若未同步更新总入口表格，会造成历史版本不可追溯 | P1 |

| 维度 | 说明 |
|------|------|
| 功能介绍 | **全局总入口**展示版本表格（版本号、迭代说明、多端链接、PRD）；**v1.0.0 版本门户**提供 PC/APP 双卡片与快捷链接。 |
| 全局总入口 | `prototype/index.html` → [GitHub Pages](https://liaoliao66.github.io/wms/prototype/index.html) |
| v1.0.0 版本门户 | `prototype/versions/v1.0.0/index.html` |
| PRD 归档 | `docs/versions/v1.0.0/prd.md` |
| 前置条件 | 无登录；静态 HTML 原型。 |
| 页面跳转 | 全局表 PC 列 → v1.0.0 门户 `#pc` → `pc_home.html`；App 列 → `#app` → `app_count_home.html`；目录 → `prototype_map.html`。 |

**PC 端入口说明**：仓库管理后台，含采购、入库出库、台账、基础配置等；侧栏导航 125+ 业务页。

**独立 App 入口说明**：iOS 18 风格移动原型；本期包含资产确认、**库场盘点 APP**（首页/扫码/任务/账外/个人中心）。

**企微 H5**：v1.0.0 暂未交付，全局总入口表格标注「待规划」。

**快捷链接**：

| 链接 | 目标 | 说明 |
|------|------|------|
| 全局总入口 | prototype/index.html | 全版本表格门户 |
| v1.0.0 版本门户 | prototype/versions/v1.0.0/index.html | PC/App 双卡片 |
| 原型页面目录 | prototype_map.html | 全站页面索引 |
...
**一级概述**：面向领用人的个人物资视图。侧栏菜单为 **领用记录**、**归还记录**（强调全量历史可查，而非仅「待办」状态）；工作台「待还超期」等指标仍沿用业务口径。
...
**与物资归还模块关系**：归还记录为领用人视角；仓库管理员在「物资归还」执行入库与作废，领用人在「待确认」状态完成确认后变为已归还。
...
| 确认人 | **借用人本人**（`return_tracking.borrower_id`）在「归还记录」点击「确认」 |
...
| 类型 | 说明 |
|------|------|
| 正常流程 | 计划审核通过且关联物资已采购入库完成后起算；列表对超期计划行高亮/Badge「领用周期超期」 |
| 边界情况 | 一单含多物资时，任一明细行超期即整单标记超期；展示「超期明细数」 |
| 异常兜底 | 尚未入库完成：不计时、不报警；分类未配置领用周期：不报警；计划额度已全部领用完毕：解除报警 |

跳转：工作台异常行 → 本列表（可带 `tab=已通过` 与超期筛选）。

#### 3.3.2 领用申请

| 维度 | 说明 |
|------|------|
| 功能介绍 | 发起物资领用请求；**审核通过后锁库预留**，仓库执行出库时**扣减在库数量**；驳回/关闭时**释放预留**。审核通过后自动生成出库待办与领用记录。 |
| 前置条件 | 可选关联已审核通过的物资计划；明细申请数量 ≤ **可用库存**（`available`）。 |
| 数据权限 | 申请人操作本人单据；仓库管理员可查看全部待出库关联单。 |
| 页面跳转 | 「新增」→ 新增页 → 添加物资；提交后回列表；审核通过后可在领用记录、物资出库查看。 |
...
| 领用记录 | 同左 | 无直接库存变更；展示待领取/已领取 |
| 归还记录 | 出库完成且需归还 | `borrowed` 增加 |
...
**说明**：列表为急件采购申请，按添加时间降序；编辑时申请单号、采购总额不可改。**列表不展示**来源计划单号、来源待采（从待采发起时仍在申请表单/详情中体现）。
...
| 前置条件 | 出库完成且物资需归还。 |
| 数据权限 | 仓库管理员操作归还/作废；领用人可在归还记录查看并确认。 |
...
| 归还操作 | 归还记录终态 | 是否入待报废池 | 作废单 | 库存影响（执行前→执行后） |
|----------|--------------|----------------|--------|---------------------------|
...
4. `归还灭失`：关联归还记录 status=待归还/已延期；不可与「损坏归还」重复核销同一资产。
...
**一级概述**：依据《物资供应商入库管理办法》，建立供应商**准入审核 → 入库建档 → 分级存续 → 年度考核 → 淘汰退出**闭环；与采购、供货、验收、评价联动。

**状态模型（两层）**：

| 维度 | 枚举 | 说明 |
|------|------|------|
| 库内状态 `pool_status` | 在库 / 暂缓入库 / 已淘汰 / 已退出 | 是否在合格供应商资源库；仅**在库**可参与采购选型 |
| 合作状态 `coop_status` | 正常 / 暂停 / 黑名单 | 在库期间的履约合作控制 |
| 供应商分级 `tier` | 核心供应商 / 合格供应商 / 备选供应商 | 入库后按重要度、体量、考核评分分级 |

**管理职责（办法第三条）**：设备技术部牵头初审与档案；综合管理部复核合规征信；财务部复核财税开票；分管领导终审。

#### 3.6.1 入库审核

| 维度 | 说明 |
|------|------|
| 功能介绍 | 供应商入库五级流程：**资料申报 → 设备技术部初审（3 工作日）→ 联合复核（综合管理部 + 财务部，各 3 日）→ 现场核查（核心/大额/定制类）→ 终审（分管领导，3 日）→ 入库建档**；结论：准予入库 / 暂缓入库 / 不予入库。 |
| 前置条件 | 供应商提交完整申报资料（营业执照、法人、开户开票、资质证照、专项资质等，见办法第三章）。 |
| 数据权限 | 设备技术部接收与初审；综合管理部、财务部联合复核；分管领导终审。 |
| 页面跳转 | 列表「审核」→ 审核详情；「登记申报」→ 登记表单；准予入库后生成供应商编码并写入合格库。 |

**禁止入库情形**（办法第二章第三条）：虚假资料、经营异常、重大质量/安全事故、历史违约、挂靠空壳、商业贿赂等，直接不予入库或剔除。

#### 3.6.2 供应商列表

| 维度 | 说明 |
|------|------|
| 功能介绍 | 合格供应商资源库（一户一档）：基础信息、资质开票、分级、库内/合作状态；关联供货、验收、退货与评价。 |
| 前置条件 | 已通过入库审核或历史迁移数据。 |
| 数据权限 | 设备技术部、采购、管理员可维护档案；采购选型仅可选**在库 + 合作正常**。 |
| 页面跳转 | 「新增」→ 档案表单（已通过审核供应商）；「查看」→ 详情（资质档案 / 审核记录 / 业务 Tab）。 |

**档案变更**：营业执照、资质证书、账户信息等变更须 15 个工作日内报备并更新档案（办法第五章第一条）。

#### 3.6.3 供应商评价

| 维度 | 说明 |
|------|------|
| 功能介绍 | 年度考核评审（办法第五章第三条）：**产品质量、供货时效、价格优势、售后服务、履约能力、合规经营**六维加权评分；结果作为升级/降级/淘汰依据。 |
| 前置条件 | 供应商已在库；评价指标与权重已配置（§17.10.8）。 |
| 数据权限 | 设备技术部组织；评价人创建；审批人审核。 |
| 页面跳转 | 「新增评价」→ 评价表单；审批通过后回写最新等级与综合分。 |

**淘汰规则（办法第六章）**：质量连续不合格、多次逾期断供、恶意抬价、售后推诿、资质过期、串标贿赂、年度考核不合格等 8 类情形，移出合格库并 1～3 年禁止重新准入。
...
| 计量单位 | 默认继承父级，可改 | ✓ | ✓ |
| 盘点类型 | 多选，默认继承父级 | ✓ | ✓ |
| 是否归还 | 默认继承父级，可改；类资产/固定资产可配置 | ✓ | 默认否 |
| 领用周期（天） | **各级均可配置**；默认继承上级，可改；正整数（天）。自采购入库完成日起算，超时且未领完则报警 | ✓ | ✓ |
...
| 维度 | 说明 |
|------|------|
| 配置范围 | 物资大类、一级子类、二级及以下子类均需可维护 |
| 继承规则 | 与「是否归还」相同：新建时带入上级当前值，允许本级修改并落库；不强制 ≤ 上级 |
| 取值优先级 | 业务判定取物资所属**叶子分类**上的领用周期（本级已改则用本级，否则为继承后保存值） |
| 计时起点 | 该计划明细对应物资的**采购入库完成日**（入库单完成日期） |
| 报警条件 | `当前日期 − 入库完成日 > 领用周期（天）`，且该计划明细仍存在**未领用完**额度 |
| 解除条件 | 未领用额度为 0，或计划作废/关闭（若业务支持） |
| 提醒入口 | **工作台**异常物资提醒 Tab「领用周期」；**物资计划**列表超期标识 |

```mermaid
flowchart TD
  A[分类配置领用周期] --> B[物资计划审核通过]
  B --> C[采购入库完成]
  C --> D[开始计时]
  D --> E{当前-入库完成日 大于 领用周期?}
  E -->|否| F[正常]
  E -->|是| G{仍有未领用额度?}
  G -->|否| F
  G -->|是| H[工作台 + 物资计划报警]
...
**风险项**：勿与「借用周期」混用；未入库不计时；空值不报警（大类建议必填，子类默认可继承）。
...
**一级概述**：对**库外**（不在仓库货位内）的 **固定资产、类资产** 开展盘点。PC 创建计划并发布任务；执行人通过**企微工作台 H5**（主入口）/PC 确认在场与数量；先输出**盘点结果**，确认无误后生成**盘点报告**；盘亏/盘盈/归属变更在**差异处理**中独立推进。

**与库场盘点区别**：

| 维度 | 库场盘点（§3.8） | 库外盘点（本节） |
|------|------------------|------------------|
| 对象 | 在库物资（货位账本） | 库外固定资产（一物一码）；库外类资产（编码+负责人+使用地点） |
| 范围含义 | 仓库/分区/货架 | **应盘哪些物资**（大类/子类/地点/清单） |
| 任务拆分 | 按货架 | **按指定负责人/归属人**（执行人） |
| 整单任务 | 可有库场侧能力 | **不可改派**（任务为一次完整物资确认过程） |
| 盘亏处理 | 调货位/调整库存 | **走物资作废（报废）等差异处理** |
| 盘盈 | 调整库存 | **账外资产审批入账**（不在本行加大账存） |
...
| 盘点范围 | 界定**应盘哪些库外物资**，与执行人无关。支持：全部库外资产、指定物资大类、指定子类、指定使用地点、指定物资清单。 |
| 前置条件 | 库外资产台账已有数据；用户有库外盘点权限。 |
| 数据权限 | 盘点管理员、部门负责人、系统管理员。 |
| 页面跳转 | 列表「新增」→ 计划表单；行内「任务/结果/报告」跳转对应模块。 |

**发布规则**：按口径日冻结应盘物资清单 → 点击**发布**后计划直接进入**盘点中** → 出库领用人（归属人）企微 H5/PC 确认。  
**结束规则**：仅**物资管理员**可手动**结束**，结束后计划进入**已结束**；领用人不可再确认；**仅已结束**计划可查看/生成盘点报告。  
**缺地点发布**：类资产台账缺使用地点时**仍可发布**；该物资须在盘点中资产确认补填地点后方可完成本行。

#### 3.9.2 库外盘点任务

| 维度 | 说明 |
|------|------|
| 功能介绍 | 按执行人拆分的任务列表；展示待盘/已确认等进度。固资另计待我确认、已转派。 |
| 任务改派 | **整单任务不可改派**；口径日后负责人变更亦不自动换执行人，本期由原执行人完成。 |
| 固资转派 | 仅支持任务内**单件物资转派**（接受/拒绝/撤回）；不支持整单任务转派。 |
| 类资产转派 | **不支持**；由指定负责人自盘。 |
| 页面跳转 | 「查看」→ 任务详情 / 企微执行页。 |

**任务状态（执行人侧）**：进行中 → 已完成（本人侧待盘=0，且无固资待我确认/已转派待办）。

#### 3.9.3 库外盘点结果与报告

| 维度 | 说明 |
|------|------|
| 盘点结果 | 汇总应盘/已盘/盘亏候选/账外；可导出；**已结束**计划可进入结果页。 |
| 盘点报告 | **仅已结束**计划可查看/生成报告并冻结，用于审计归档；与差异处理进度**独立**。 |
| 差异处理 | 盘亏→单独差异逻辑（如作废）；盘盈（账外）→审批入账；固资归属变更→对方确认/拒绝。 |
| 结束后纠错 | 计划**已结束**并出报告后，**不回改盘点行数量**；异常走**独立异常/差异处理**，不存在「改在场/丢失后重开盘点差异」路径。 |
...
| 状态 | 说明 | 领用人 | 管理员主要操作 |
|------|------|--------|----------------|
| 草稿 | 未发布，可编辑 | 不可确认 | 编辑、删除、发布 |
| 盘点中 | 手动发布即进入，确认进行中 | 可确认 | 盘点进度、结束 |
| 已结束 | 盘点已关闭 | 只读查看 | 结果、生成/查看报告、差异 |

**报告可见性**：盘点中**不可**查看/生成报告；**已结束**后方可。
...
> 优先级：均为 **P0**（本期必做）。与固资「一物一码 · 在场/丢失/转派」分流；企微原型见 `pages/wecom/`。

##### 正常流程

1. 发布时按口径日冻结：`物资编码 + 指定负责人 + 使用地点 + 账存数量`（缺地点仍可进任务）。  
2. 任务下发给指定负责人（整单不可改派；类资产不可转派）。  
3. 执行人打开该类资产，维护**多地点清单**并录入各点 **在场数 / 丢失数**。  
4. 各地点满足「地点名称已填且在场+丢失=该地点账存」后提交；地点变更**回写资产台账**。  
5. 全部应盘完成后出盘点结果/报告；丢失数形成盘亏候选；多出走账外登记。

```mermaid
flowchart TD
  A["发布计划冻结应盘"] --> B["按指定负责人生成任务"]
  B --> C["类资产：多地点维护"]
  C --> D["编辑地点名称"]
  C --> E["增加地点：从已有地点拆账存"]
  C --> F["删除地点：合并到另一地点"]
...
**文字解读**：类资产盘的是「各使用地点数量是否账实相符」；地点结构可在盘点中调整，但物资总账存不因增删地点而增减（增=拆、删=合并）。

##### 使用地点增删改

| 操作 | 规则 | 优先级 |
|------|------|--------|
| 编辑 | 修改地点名称；同一负责人同一物资下地点名称不可重复；提交成功后回写台账 | P0 |
| 增加 | 从**已有地点拆出账存数量**生成新地点；物资账存合计不变；新地点在场/丢失初始为 0 | P0 |
| 删除 | 须指定合并目标地点；账存及已录在场/丢失一并并入目标；**至少保留 1 个地点** | P0 |
| 缺地点 | 允许发布；确认时必须补填地点名称后方可完成本物资 | P0 |

##### 数量与完成条件

| 规则 | 说明 | 优先级 |
|------|------|--------|
| 行/地点恒等式 | 每个使用地点完成时：`在场数 + 丢失数 = 该地点账存` | P0 |
| 暂存 | 未提交前允许未拆满；提交时强制校验 | P0 |
| 禁止本行盘盈 | 不可使在场+丢失 > 账存；现场多出须走**账外登记** | P0 |
| 丢失 | 丢失数 > 0 进入盘亏候选；丢失说明建议必填 | P0 |
| 改数窗口 | 计划**盘点中**且本人未结束确认前可改；计划**已结束**后不回改盘点行 | P0 |

##### 边界情况

| 场景 | 处理 | 优先级 |
|------|------|--------|
| 同编码多地点 | 同一确认页按地点列表维护，任务列表可按编码聚合展示 | P0 |
| 仅 1 个地点 | 禁止删除（须先增地点再删，或不可删至 0） | P0 |
| 拆出数量 > 源地点账存 | 禁止 | P0 |
| 合并删除 | 目标地点账存/在场/丢失累加后，仍须满足提交恒等式（可再调整） | P0 |
...
| 风险 | 说明 | 缓解 |
|------|------|------|
| 盘中频繁拆并地点 | 操作复杂、易录错 | 确认页实时展示各地点账存与待拆分 |
| 回写台账与报告口径 | 确认地点以提交结果为准并回写 | 审计记录原地点、新地点、操作人、任务号 |

##### 与固资对照

| 维度 | 固定资产 | 类资产 |
|------|----------|--------|
| 标识 | `asset_code` | `material_code` + 负责人 + 使用地点 |
| 动作 | 在场 / 丢失 / 单件转派 | 多地点增删改 + 在场数/丢失数 |
| 转派 | 支持（任务内单件） | **禁止** |
| 盘盈 | 账外 | 账外（禁止本行加大） |
...
**一级概述**：登录后默认首页，聚合业务队列统计、**异常物资提醒**与库存经营指标；可执行待办由**平台待办中心**提供，不在本子系统重复建设。

| 维度 | 说明 |
|------|------|
| 功能介绍 | 展示业务统计卡片、需仓管跟进的异常物资清单。 |
| 前置条件 | 用户已登录。 |
| 数据权限 | 统计卡片与异常物资按角色+分管仓库过滤；领用人仅见本人相关异常（如借还超期）。 |
| 页面跳转 | 统计卡片、异常行「处理」跳转对应业务列表或详情。 |

**统计卡片（V1.5.17，单行四组布局）**：

| 分组 | 包含卡片 | 说明 |
|------|----------|------|
| **采购入库** | 待采物资、供货逾期、待验物资、待入库 | 采购到入库链路 |
| **出库归还** | 待出库、待还超期 | 领用出库与归还超期 |
| **库场盘点** | 盘点任务 | 盘点执行待办 |
| **报废处置** | 待报废、待执行作废、待处置、**待执行处置** | 作废前池→执行→处置队列 |

整行横向排列；窄屏可横向滚动，不换行。卡片点击一律进入**列表页**（带 Tab 筛选），不进表单。
...
| 待还超期 | 归还 status=已延期 | 归还记录 |
| 待报废 | 待报废池 | 待报废池 |
| 待执行作废 | 作废单 status=待执行 | 物资作废列表 |
| 待处置 | 待处置队列项数 | 待处置物资池 |
| 待执行处置 | 处置单 status=待执行 | 物资处置列表 |

**异常物资提醒（V1.5.18）**：

面向**仓库管理员**（及有分管仓库权限的角色），列举分管范围内需跟进的物资级异常，提醒尽快处理。与统计卡片（队列计数）互补：本区展示**具体物资行**及建议处理入口。

| 异常类型 | 判定规则 | 建议处理 | 跳转 |
|----------|----------|----------|------|
| 库存下限预警 | 类资产/耗材 `available < min_stock` 且 `available > 0` | 补货、急件采购 | 物资台账详情 / 待采物资 |
| 缺货 | `available = 0` 且 `min_stock > 0` | 紧急补货 | 物资台账·库存预警 Tab |
| 待报废 | `scrap_pool` 有行，或货位状态=待报废 | 发起作废 / 跟进池内物资 | 待报废池 / 作废表单 |
| 使用年限到期 | 年限到期入 `scrap_pool`（`service_life`） | 延长年限或发起作废 | 待报废池 |
| 借还超期 | `return_tracking.status=已延期` | 催还、跟进借用人 | 归还记录 |
| 领用周期超期 | 计划明细已入库完成且 `today − inbound_done_date > 分类.领用周期`，且未领用额度 > 0 | 复核计划合理性、催领或调整计划 | 物资计划列表 |

- 支持 Tab 筛选：全部 / 库存预警 / 待报废 / 借还超期 / 使用年限 / **领用周期**。
- 默认按异常严重程度排序：缺货 > 借还超期 > **领用周期超期** > 待报废 > 库存下限预警 > 使用年限到期。
- 列表上限可配置（原型展示样本行）；底栏提供分类汇总跳转链接（含物资计划超期）。
- **不含**审批待办、验收/入库执行待办（归平台待办中心）。

**导航**：侧栏「工作台」指向 `pc_home.html`；各业务页底部「返回原型入口」链回 `prototype/index.html`（全局总入口）或 `prototype/versions/v1.0.0/index.html`（版本门户）。
...
| 领用记录详情 | 领用记录 · 查看 | `mine_requisition_record` | 领用记录列表 |
...
flowchart TB
    subgraph uf3_user["泳道 · 领用人"]
        U1["领用申请"] --> U2["审核通过"] --> U3["领用记录·待领取"] --> U4["已领取"]
    end
...
| 领用记录 | §3.2.1 | 领用记录 | `mine_pending_pickup.html` | ✓ |
| 归还记录 | §3.2.2 | 归还记录 | `mine_pending_return.html` | ✓ |
...
> 本版本**不单独设计**登录、组织架构、账号管理页面；以下仅作为业务运行前提，供研发与实施对齐。

| 项 | 业务前提 | 本 PRD 处理原则 |
|----|----------|----------------|
| 登录 | 用户需先完成平台登录 | 仅作为访问前置，不设计登录页 |
| 会话 | 会话有效期、续期、退出策略由平台统一管理 | 业务页面默认复用平台登录态 |
| 组织 | 用户、部门、负责人关系由平台主数据提供 | 本 PRD 只引用组织结果，不设计维护界面 |
| 仓库绑定 | 仓库管理员/验收人员需绑定可见仓库范围 | 仅影响业务数据范围，不单独设计配置页 |
| 默认初始化 | 默认管理员、默认仓库、默认组织等由实施初始化完成 | 不在业务原型中表现 |

### 6.8 操作留痕与审计（平台承接）

> 本版本**不单独设计审计中心页面**；仅约束关键业务动作必须可追溯。

| 审计对象 | 最低要求 | 说明 |
|----------|----------|------|
| 入库/出库/归还确认 | 记录操作人、时间、单据号、关键数量变化 | 用于库存追责 |
| 验收/退货/作废/处置 | 记录处理结果、关联单据、关键说明 | 用于异常追踪 |
| 库存调整/盘点差异处理 | 记录调整前后值、原因、处理人 | 用于账实核查 |
| 业务查看修复类操作 | 可选留痕 | 视平台能力决定 |

**保留原则**：审计日志保留不少于 3 年；查询、导出、归档能力由平台统一承接，不在本 PRD 设计专属界面。
...
| 我的物资 | 领用记录 | 领用领取全状态跟踪（待领取/已领取） |
| 我的物资 | 归还记录 | 借出归还全生命周期（含待确认） |
| 物资申请 | 物资计划 | 需求计划编制与审批 |
| 物资申请 | 领用申请 | 领用申请与审批 |
| 采购管理 | 待采物资 | 急件待采购清单 |
...
> 本章仅用于描述**业务角色视角下的数据可见范围与操作边界**。菜单权限、按钮权限、字段脱敏、审批节点分配等能力由平台统一配置，不在本 PRD 设计配置界面。

### 8.1 业务角色可见边界（菜单/操作视角）
...
> ✓=业务上可见/可操作，—=业务上不应开放，「本人相关」=仅关联本人的业务数据。最终权限落地由平台权限中心配置。
...
| 能力 | 是否在本 PRD 详细设计 | 处理方式 |
|------|------------------------|----------|
| 菜单权限配置 | 否 | 由平台权限中心承接 |
| 按钮权限配置 | 否 | 由平台权限中心承接 |
| 数据行权限配置 | 否（仅保留业务原则） | 本章说明“谁看什么”，具体配置不展开 |
| 字段脱敏规则 | 否 | 若后续涉及隐私字段，另起专题版本 |
| 审批节点指派 | 否 | 平台流程中心承接 |
...
| 借用周期 | 分类/子类配置（天） | 同左 | — |
| 领用周期 | 分类各级配置（天），继承上级可改 | 同左 | 入库完成日起算，超时未领完报警 |
...
| 借还超期 | 每日批处理，`due_date < today` 且未归还 | 工作台卡片 → 归还记录 |
| 归还待确认 | 仓管归还入库 | 归还记录 Tab + 工作台待办（借用人） |
| 归还待确认超时 | 7 日未确认 | 工作台提醒（不自动确认） |
| 库存下限预警 | 耗材/类资产 `available < min_stock` | 物资清单预警列、**工作台异常物资提醒** |
...
| 页面 | 状态 | 可用操作 | 目标 |
|------|------|----------|------|
| 领用记录 | 待领取 | 查看 **[弹窗]** | `mine_requisition_record` |
| 领用记录 | 已领取 | 查看 **[弹窗]** | `mine_requisition_record` |
| 归还记录 | 待归还/已延期 | 查看 | 归还单详情 |
| 归还记录 | 待确认 | 查看、确认 | `mine_return_confirm` |
| 归还记录 | 部分归还 | 查看 | 归还单详情 |
| 归还记录 | 已归还/已作废 | 查看 | 归还单详情 |
...
| 分类管理 | 已引用 | 编辑、停用 | 不可删除 |
| 供应商列表 | 在库+正常 | 查看、编辑 | 供应商表单 |
| 供应商列表 | 在库+暂停/黑名单 | 查看、编辑 | 暂停不可被选入采购/评价 |
| 供应商列表 | 已淘汰/已退出 | 查看 | 只读档案，展示退出原因 |
| 入库审核 | 资料申报/初审/复核/核查/待终审 | 查看、审核 | 审核详情 |
| 入库审核 | 暂缓入库 | 查看、补充资料 | 重新提交审核 |
| 入库审核 | 不予入库 | 查看 | 只读 |
...
**页面说明**：PC 登录默认页；**统计卡片**（四组单行队列计数）+ **异常物资提醒**（物资行级异常）。不含作业待办、快捷入口与库存概览（台账类查询走侧栏菜单）。原型：`pc_home.html`。
...
| 分组 key | 分组标题 | 卡片（顺序） |
|----------|----------|--------------|
| `procure` | 采购入库 | 待采物资、供货逾期、待验物资、待入库 |
| `outbound` | 出库归还 | 待出库、待还超期 |
| `count` | 库场盘点 | 盘点任务 |
| `scrap` | 报废处置 | 待报废、待执行作废、待处置、**待执行处置** |
...
| 待还超期 | 借还超期行数 | `return_tracking` | `status=已延期`；仓管=分管仓库全量，领用人=本人 | 卡片；跳转归还记录 `tab=已延期` |
...
| 待执行处置 | 处置单待执行数 | `disposal_order` | `status=待执行` | 卡片；跳转处置列表 `tab=待执行处置` |

卡片副文案统一「查看列表 →」；点击目标为列表页，不进表单。

#### 17.1.2 异常物资提醒

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 异常类型 | 业务分类 | 聚合规则 | 库存下限预警/缺货/待报废/使用年限到期/借还超期/领用周期超期 | 左侧彩色 Badge |
| 物资编码 | 编码或资产编码 | `material_catalog` / `asset` / 计划明细 | — | 默认列 |
| 物资名称 | 名称 | `material_catalog.name` | — | 默认列 |
| 规格型号 | 规格 | `material_catalog.spec` | — | 默认列 |
| 仓库/位置 | 货位、借出地或计划单号 | `inventory_location` / `return_tracking` / `material_plan` | 领用周期超期可展示计划单号 | 默认列 |
| 异常说明 | 可读描述 | 模板拼接 | 如「可用 15 桶，低于下限 20 桶」；领用周期：「入库完成已超 N 天，未领用 X」 | 主文案 |
| 处理 | 建议操作入口 | — | 链至台账详情/待报废池/归还记录/物资计划等 | 右侧链接 |

**筛选 Tab**：全部 / 库存预警（含缺货）/ 待报废 / 借还超期 / 使用年限 / 领用周期。

**数据范围**：仓库管理员=分管 `warehouse_ids` 内物资；系统管理员=全量；领用人仅「借还超期」且 `borrower=本人`；领用周期超期对计划编制人/物资管理员可见。

**排序**：缺货 > 借还超期 > 领用周期超期 > 待报废 > 库存下限预警 > 使用年限到期；同优先级按发现时间倒序。

**底栏链接**：库存预警台账、待报废池、超期归还、物资计划。

#### 17.1.3 功能权限（操作项）

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 查看统计卡片 | `wms:dashboard:view` | 按 §3.11 角色矩阵 | 有对应业务权限 | 各模块列表 |
| 查看异常物资 | `wms:dashboard:anomaly:view` | 仓管、管理员；领用人见本人超期 | 有异常行 | 列表内处理链接 |
...
#### 17.3.1 领用记录（mine_pending_pickup）
...
#### 17.3.2 归还记录（mine_pending_return）
...
| 领用周期提醒 | 是否超期 | 按分类领用周期计算 | 任一明细超期则 Badge「领用周期超期」 | 默认列 |
...
| 业务状态 | 执行态 | `biz_status` | 待提交/审核中/审核通过 | Tab col 5 |
...
| 编辑 | `wms:outside:plan:edit` | 草稿 | 计划表单 |
| 发布 | `wms:outside:plan:publish` | 草稿 | 发布 → **盘点中** |
| 盘点进度 | `wms:outside:task:view` | 盘点中及以后 | `outside_count_task_list` |
| 结束 | `wms:outside:plan:close` | 盘点中 | 结束 → **已结束** |
| 结果 | `wms:outside:result:view` | 已结束 | `outside_count_result` |
| 报告 | `wms:outside:report:view` | **已结束** | `outside_count_report_list` / `detail` |
| 差异 | `wms:outside:diff:view` | 已结束 | `outside_count_diff_list` |
...
> 业务依据：《物资供应商入库管理办法》。库内状态与合作状态分层管理；采购选型约束：**在库 + 合作正常**。

#### 17.9.1 入库审核（supplier_admission_list）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 申请单号 | RK | `supplier_admission.no` | 系统生成 | 默认列 |
| 供应商名称 | 名称 | 申报 | — | 默认列 |
| 统一社会信用代码 | 信用代码 | 申报 | 18 位校验 | 默认列 |
| 供应商类型 | 类型 | 申报 | — | 默认列 |
| 当前环节 | 流程节点 | `stage` | 申报/初审/联合复核/现场核查/待终审/暂缓/不予 | Badge |
| 申请日期 | 日期 | `submit_date` | — | 默认列 |
| 经办人 | 对接人 | 设备技术部 | — | 默认列 |
| 需现场核查 | 是/否 | 规则 | 核心/大额/定制类 | 默认列 |

**筛选与搜索**

| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| Tab | 全部/资料申报/设备技术部初审/联合复核/现场核查/待终审/暂缓入库/不予入库 | `tab` | 按当前环节 |
| 下拉 | 供应商类型 | `filters.type` | 全部/设备配件/耗材/电气/办公等 |
| 关键字 | 搜索框 | `keyword` | 申请单号、名称、信用代码 |

**功能权限（操作项）**

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 登记申报 | `wms:supplier:admission:create` | 设备技术部 | — | `supplier_admission_form.html` |
| 审核 | `wms:supplier:admission:audit` | 初审/复核/终审角色 | 非终态 | `supplier_admission_detail.html` |
| 查看 | `wms:supplier:admission:view` | 采购/管理员 | 全部 | 审核详情 |

#### 17.9.2 供应商列表（supplier_list）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 供应商编码 | 编码 | `supplier.code` | 入库建档分配 | 默认列 |
| 供应商名称 | 名称 | `name` | — | 默认列 |
| 库内状态 | 在库/暂缓/已淘汰/已退出 | `pool_status` | 入库/淘汰流程 | Badge |
| 合作状态 | 正常/暂停/黑名单 | `coop_status` | 下拉 col | Badge |
| 分级 | 核心/合格/备选 | `tier` | 考核与体量 | Badge |
| 统一社会信用代码 | 信用代码 | `credit_code` | 申报 | 默认列 |
| 联系人 | 联系人 | `contact` | — | 默认列 |
| 联系电话 | 电话 | `phone` | 脱敏展示 | 默认列 |
| 最新等级 | 评价等级 | 评价回写 | 评价通过后更新 | 默认列 |
| 入库日期 | 建档日期 | `admission_date` | 终审通过日 | 默认列 |

**筛选与搜索**

| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| 下拉 | 库内状态 | `filters.poolStatus` | 全部/在库/暂缓入库/已淘汰/已退出 |
| 下拉 | 合作状态 | `filters.coopStatus` | 全部/正常/暂停/黑名单 |
| 下拉 | 分级 | `filters.tier` | 全部/核心/合格/备选 |
| 关键字 | 搜索框 | `keyword` | 编码、名称、信用代码、联系人 |

**功能权限（操作项）**

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 新增 | `wms:supplier:create` | 设备技术部/管理员 | — | `supplier_form.html` |
| 查看 | `wms:supplier:view` | 采购 | 全部 | `supplier_detail.html` |
| 编辑 | `wms:supplier:edit` | 设备技术部 | 非已退出 | `supplier_form.html?mode=edit` |

#### 17.9.3 供应商新增/编辑（supplier_form）
...
| 供应商简称 | 简称 | 手工 | 否 | — | 保存 |
| 供应商类型 | 类型 | 选择 | 是 | — | 保存 |
| 统一社会信用代码 | 信用代码 | 手工 | 是 | 18 位 | 保存 |
| 法定代表人 | 法人 | 手工 | 是 | — | 保存 |
| 联系人 / 联系电话 | 联系 | 手工 | 是 | 手机格式 | 保存 |
| 经营地址 | 地址 | 手工 | 是 | — | 保存 |
| 开户银行 / 对公账号 | 账户 | 手工 | 是 | — | 保存 |
| 开票抬头 / 税号 | 开票 | 手工 | 是 | — | 保存 |
| 专项资质类型 | 多选 | 选择 | 否 | ISO/3C/危险品等 | 保存 |
| 合作状态 | 状态 | 选择 | 是 | 正常/暂停/黑名单 | 保存 |
| 供应商分级 | 分级 | 选择 | 否 | 核心/合格/备选 | 保存 |

> 新供应商须先走入库审核；本表单用于已通过审核供应商的档案维护。

#### 17.9.4 供应商详情（supplier_detail）

**Tab 与内容**

| Tab | 内容 |
|-----|------|
| 基础信息 | 简称、联系、地址、入库日期、最近评价单号 |
| 资质档案 | 申报资料清单（营业执照、开户许可、资质证书等）及有效期 |
| 审核记录 | 入库审核轨迹（申报→初审→联合复核→现场核查→终审） |
| 淘汰退出 | 已淘汰/已退出时展示原因、时间、重新准入限制 |
| 绩效评价 | 已通过评价记录 |
| 供货/验收/退货 | 关联业务单据 |

#### 17.9.5 供应商评价（supplier_eval_list）
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 新增 | `wms:supplier:eval:create` | 设备技术部/采购 | — | `supplier_eval_form.html` |
...
树形+子类表格；资产类子类列：**序号、分类编码、分类名称、计量单位、是否需要盘点、是否需要使用年限、领用周期（天）、备注**。耗材子类列同步增加**领用周期（天）**。大类详情区展示领用周期（继承规则同是否归还）。

**表单字段（大类 / 各级子类）**

| 字段名 | 含义 | 数据来源 | 必填 | 校验规则 | 提交逻辑 |
|--------|------|----------|------|----------|----------|
| 领用周期（天） | 计划入库后允许消化的自然日上限 | 默认继承上级，可改 | 大类建议必填；子类有上级时可继承 | 正整数 ≥1 | 保存本级值；业务取叶子分类 |
...
**弹窗说明**：领用记录「查看」；展示领用申请+出库信息。尺寸 lg。参数：领用单 `no`、`back`。
...
2. **交互原型 v1.0.0**（`prototype/`）：当前归档为 162 页静态 HTML，GitHub Pages 部署
   - 全局总入口：`prototype/index.html`
   - v1.0.0 版本门户：`prototype/versions/v1.0.0/index.html`
   - 页面目录：`prototype/versions/v1.0.0/pages/`
   - 生成脚本：`prototype/scripts/generate-pages.mjs`
   - 在线地址：https://liaoliao66.github.io/wms/prototype/index.html
...
| V1.3.0 | 2026-06-16 | 对齐原型迭代：① V1.0 范围调整，库场盘点移至 V1.1；② 菜单更名领用记录/归还记录，补充待确认状态；③ 详情弹窗规范（流水/货位/计划/领用）；④ 仓库台账树形筛选与差异化操作列；⑤ 物资清单分类树+动态列+类资产库存必填；⑥ 仓库配置移除重复 Tab，明确货位码/资产码；⑦ 待采物资「查看计划」改弹窗 |
...
| V1.5.20 | 2026-07-07 | 供应商管理对齐《物资供应商入库管理办法》：① 新增**入库审核**流程页（五级审核、暂缓/不予入库）；② 供应商列表扩充库内状态/分级/信用代码；③ 档案表单与详情 Tab（资质档案、审核记录、淘汰退出）；④ 评价维度调整为六维（产品质量/供货时效/价格/售后/履约/合规）；§3.6、§16.5、§17.9 同步 |
| V1.5.24 | 2026-07-28 | 采购申请列表移除「来源计划单号」「来源待采」两列；搜索关键字调整为申请单号、提交人；§3.4.2、§17.5.2 与原型同步（表单仍保留待采带入字段，见 §17.5.3） |
| V1.5.23 | 2026-07-24 | 盘点模块物理拆分：库外盘点 → v1.1.0；库内（库场）盘点 → v1.2.0；本版本侧栏与原型页移除盘点入口 |
| V1.5.22 | 2026-07-23 | 库外盘点计划状态简化为三态：**草稿 → 盘点中 → 已结束**；手动**发布**即进入盘点中；**仅已结束**可查看/生成报告；§3.9、§17.8 与 PC/企微原型同步 |
| V1.5.21 | 2026-07-16 | 按最新 rules 优化归档版 PRD：① 明确 v1.0.0 版本范围、待规划与平台承接边界；② 原型入口切换为“全局总入口 + 版本门户”双层结构；③ 将认证/审计/权限内容收敛为平台承接说明；④ 文末补标准确认表单说明 |
| V1.5.19 | 2026-07-06 | 工作台移除**库存概览**区块（在库指标、分管仓库快览、台账快捷按钮）；查询走侧栏物资台账；§3.11、§17.1 同步 |
| V1.5.18 | 2026-07-06 | 工作台重构：① 移除作业待办/快捷入口（归平台）；② 新增**异常物资提醒**清单（库存预警/缺货/待报废/使用年限/借还超期）；③ 报废处置组补「待执行处置」卡；④ 库存概览升格全宽；§3.11、§17.1 同步 |
| V1.5.17 | 2026-07-06 | 工作台统计卡片改为**单行四组**布局：采购入库/出库归还/库场盘点/报废处置；§3.11、§17.1.1 同步 |
| V1.5.16 | 2026-07-06 | 我的物资侧栏菜单更名：领取记录→**领用记录**、借还记录→**归还记录**；§1.4/§3.2/§17.3 与原型 `WMS_NAV` 同步 |
...
| AC-03 | P0 | 领用 → 出库 → 领用记录 | 库存充足 | 领用审核 → 出库 → 查看领用记录 | 待领取→已领取；货位 qty 扣减；出库流水可查 | §3.2.1 |
...
| AC-DASH-20 | P0 | 异常物资提醒 | 仓管登录工作台 | 见异常物资表；Tab 可筛；处理链至对应页 | §3.11、§17.1.2 |
| AC-15 | P1 | 工作台盘点任务卡片 | 存在进行中盘点任务 | 登录工作台 | 展示「盘点任务」卡片；点击跳转盘点任务列表 | §3.11、§17.7 |
| AC-16 | P1 | 权限-领用人数据范围 | 两领用人账号 | A 登录查归还记录 | 仅见本人记录 | §8.1 |
...
### 18.5 GenUI Project Confirmation（归档版）

> 说明：以下区块按最新 rules 保留为标准确认表单模板。`v1.0.0` 已完成交付归档，因此本区块用于后续版本迭代时复用，不代表当前版本继续生成原型。

```markdown
✅ **PRD与交互流程已生成**：`/workspace/docs/versions/v1.0.0/prd.md`
-----------------------------------------------------------------------
...
[ 🧩 确认并生成企业微信内嵌H5原型 ] (回复: "确认企微H5")
[ 💻 确认并生成 PC 端 Web 原型 ]   (回复: "确认PC端")
[ 💻/🚀 确认并生成 PC 和APP 原型 ]   (回复: "两者都需要")
[ 💻/🧩 确认并生成 PC 和企微H5原型 ] (回复: "PC+企微H5")
[ 🛠️ 修改需求文档 ]               (回复: "修改: <您的意见>")
[ ❌ 拒绝并重写 ]                 (回复: "重写")
-----------------------------------------------------------------------
```
