...
| 产品版本 | V1.0.0 |
| **版本更新时间** | 2026-07-28 |
| **版本迭代说明** | 基础业务交付；库外盘点迁至 v1.1.0，库内（库场）盘点迁至 v1.2.0 |
| 部署区域 | 湖北黄冈武穴 |
| 目标平台 | PC Web |
...
- 支持 Tab 筛选：全部 / 库存预警 / 待报废 / 借还超期 / 使用年限。
- 默认按异常严重程度排序：缺货 > 借还超期 > 待报废 > 库存下限预警 > 使用年限到期。
- 列表上限可配置（原型展示 8 条）；底栏提供分类汇总跳转链接。
...
| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 异常类型 | 业务分类 | 聚合规则 | 库存下限预警/缺货/待报废/使用年限到期/借还超期 | 左侧彩色 Badge |
| 物资编码 | 编码或资产编码 | `material_catalog` / `asset` | — | 默认列 |
| 物资名称 | 名称 | `material_catalog.name` | — | 默认列 |
| 规格型号 | 规格 | `material_catalog.spec` | — | 默认列 |
| 仓库/位置 | 货位或借出地 | `inventory_location` / `return_tracking` | — | 默认列 |
| 异常说明 | 可读描述 | 模板拼接 | 如「可用 15 桶，低于下限 20 桶」 | 主文案 |
| 处理 | 建议操作入口 | — | 链至台账详情/待报废池/归还记录等 | 右侧链接 |

**筛选 Tab**：全部 / 库存预警（含缺货）/ 待报废 / 借还超期 / 使用年限。

**数据范围**：仓库管理员=分管 `warehouse_ids` 内物资；系统管理员=全量；领用人仅「借还超期」且 `borrower=本人`。

**排序**：缺货 > 借还超期 > 待报废 > 库存下限预警 > 使用年限到期；同优先级按发现时间倒序。

**底栏链接**：库存预警台账、待报废池、超期归还。
