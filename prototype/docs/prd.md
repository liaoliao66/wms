...
- **验收审核**（侧栏）：对每次发起的验收记录进行审核；「查看」打开验收记录详情，「审核」打开审核弹窗
- **批量验收**：本期不做
...
| 前置条件 | 验收记录已提交、待审核。 |
| 页面跳转 | 「查看」→ 验收记录详情；「审核」→ `warehouse_acceptance_audit_form`（审核弹窗，带 `no`、`supplyNo`、`back`）。 |
...
| 入库状态 | 待入库/部分/已入库 | `inbound_status` | Tab col 13 | Badge |
