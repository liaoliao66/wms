...
| **库场盘点** | 盘点任务 | 盘点执行待办 |
| **报废处置** | 待报废、待执行作废、待处置 | 作废前池→执行→处置队列 |
...
**页面说明**：PC 登录默认页；统计卡片按业务**四组单行**聚合待办数量，作业待办为可执行队列（非完整列表）。原型：`pc_home.html`。
...
| 待还超期 | 借还超期行数 | `return_tracking` | `status=已延期` | 卡片；跳转归还记录 |
