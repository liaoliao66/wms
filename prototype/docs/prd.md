...
| 物资管理 | 待验、验收、验收记录、入库、出库、归还、退货、作废/待报废池 |
...
| 物资管理 | P0 | 待验、验收、入出库、归还、退货、作废 | §17.6 |
...
> 库场盘点（计划→任务→执行→差异→调整）已纳入 §2.4、§3.9 本期范围。
...
| 维度 | 说明 |
|------|------|
| 功能介绍 | 集中展示审批、供货、盘点、借还超期等通知流水；支持已读/未读标记；可跳转业务单据。 |
| 前置条件 | 用户已登录。 |
| 数据权限 | 按当前用户相关通知过滤；管理员可见全量（可选）。 |
| 页面跳转 | 点击消息 → 对应业务列表/详情；与工作台待办互补（工作台=可执行，消息中心=历史通知）。 |
| 原型文件 | `purchase_message.html`（侧栏可通过采购管理入口或工作台链接进入） |
...
> 原型页 `planning_inside.html` / `planning_outside.html`；字段与操作见 **§17.8**。

| 维度 | 说明 |
|------|------|
| 功能介绍 | 按场内/场外维度展示物资分布与数量汇总，辅助盘点与调拨决策。 |
| 前置条件 | 台账有数据；使用地点已配置（场外）。 |
| 数据权限 | 仓库管理员、管理员可见分管范围。 |
| 页面跳转 | 场内/场外 Tab 切换；可跳转物资台账、仓库台账。 |
...
**作业待办列表**：聚合待采、验收、入库、出库、供货逾期、作废、待报废、归还待确认、**盘点任务**等执行队列，每项直达对应操作页。
...
| 计划采购申请 | 移动入口 | ✓ | 计划性采购 |
| 盘点首页 | app_count_home | ✓ | 当前计划、待办任务 |
| 任务列表 | app_count_task_list | ✓ | 盘点任务 |
| 盘点执行 | app_count_execute | ✓ | 录入实盘 |
| 扫码盘点 | app_count_scan | ✓ | 扫码入口 |
| 账外资产登记 | app_count_offbook | ✓ | 账外登记 |
| 个人中心 | app_count_profile | ✓ | 任务入口、PC 跳转 |
...
| 库场盘点 | §3.9 | 库场盘点 | `count_*.html` / `app_count_*` | ✓ |
...
| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 待办类型 | 业务分类标签 | 聚合规则 | 待采/验收/入库/出库/供货/作废/待报废/盘点 | 左侧彩色 Tag |
...
| 计划明细-物资编码 | 明细行 | 物资清单选择 | 是 | ≥1 行 | 提交校验 |
| 计划明细-数量 | 需求数量 | 手工 | 是 | >0 | 提交校验 |
| 计划明细-需求日期 | 需求日 | 手工 | 是 | ≥今天 | 提交校验 |
...
| 申请 | `wms:purchase:request:create` | 采购人员 | 待申请 | `purchase_pending_apply.html` |
...
#### 17.5.4 计划采购申请（purchase_plan_apply）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 申请单号 | PC 单号 | `plan_procure_request.no` | — | 默认列 |
| 采购方式 | 直采/询价/招标 | `procure_method` | — | 默认列 |
| 参考总额 | 参考金额 | 明细 | — | 默认列 |
| 申请日期 | 日期 | `created_at` | — | 默认列 |
| 审批状态 | 审批态 | `status` | 见 §17.0.3 | 下拉 col 4 |

**筛选与搜索**

| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| Tab | 全部/计划申请/急件申请 | `tab` | 申请类别 |
| 下拉 | 审批状态 | `filters.status` | 全部/草稿/审核中/审核通过 |
| 关键字 | 搜索框 | `keyword` | 申请单号、采购方式 |

**功能权限（操作项）**

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 新增 | `wms:purchase:plan:create` | 采购/管理员 | — | `purchase_plan_form.html` 枢纽 |
| 查看/编辑/提交/审核/删除 | `wms:purchase:plan:*` | 采购/审批 | 见 §16.3 | 直采/询价招标页 |

#### 17.5.5 计划采购表单（purchase_plan_direct / purchase_plan_inquiry_bid）

**表单字段清单**

| 字段名 | 含义 | 数据来源 | 必填 | 校验规则 | 提交逻辑 |
|--------|------|----------|------|----------|----------|
| 申请单号 | PC | 系统 | 否 | 只读 | 保存生成 |
| 选定供应商 | 供应商 | 选择 | 直采必填 | 供货状态=正常 | 保存/提交 |
| 采购方式 | 询价/招标 | 选择 | 询价招标必填 | 枚举 | 提交 |
| 参考总额 | 金额 | 明细/手工 | 直采必填 | ≥0 | 提交 |
| 截止日期 | 截止日 | 手工 | 询价招标必填 | ≥今天 | 提交 |
| 采购明细 | 物资行 | 添加物资 | 是 | ≥1 行 | 提交校验 |
...
#### 17.5.8 消息中心（purchase_message）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 消息标签 | 分类 | 消息模板 | 待审批/待入库/超期提醒等 | Tag |
| 消息内容 | 摘要 | 业务单据 | 模板+单号 | 主文案 |
| 时间 | 推送时间 | `created_at` | 相对时间 | 副文案 |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 点击消息 | `wms:purchase:message:view` | 对应业务角色 | 未读/已读 | 关联业务页 |
...
| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 验收单号 | 验收记录号 | `acceptance_record.no` | — | 默认列 |
| 物资供货单号 | 供货单 | — | — | 默认列 |
| 供应商名称 | 供应商 | — | — | 默认列 |
| 物资编码 | 编码 | — | — | 默认列 |
| 物资名称 | 名称 | — | — | 默认列 |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 入库 | `wms:warehouse:inbound:create` | 仓管 | 待入库/部分入库 | 固资/类资产/耗材入库表单 |
...
| 入库货位 | 仓库/货架/层 | 选择/扫码 | 是 | 固资每行 qty=1 | 提交扣增台账 |
| 入库说明 | 备注 | 手工 | 否 | ≤500 字 | — |
| 入库照片 | 图片 | 上传 | 否 | — | — |
...
| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 新增 | `wms:warehouse:refund:create` | 仓管 | — | 退货枢纽页 |
| 退货/查看 | `wms:warehouse:refund:*` | 仓管 | 未完结 | 分类退货表单 |
...
#### 17.8.1 场内（planning_inside）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 物资编码 | 编码 | `inventory_location` | 仓库内 | 默认列 |
| 物资名称 | 名称 | 主数据 | — | 默认列 |
| 仓库 | 仓库 | 结构 | — | 默认列 |
| 分区 | 分区 | 结构 | — | 默认列 |
| 货架 | 货架 | 结构 | — | 默认列 |
...
| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| 关键字 | 搜索框 | `keyword` | 物资编码、物资名称、仓库 |

**功能权限（操作项）**

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 查看 | `wms:planning:inside:view` | 管理员/仓管/计划 | 全部 | 台账详情 |

#### 17.8.2 场外（planning_outside）

**列表字段清单**

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 列表展示 |
|--------|------|----------|---------------|----------|
| 物资编码 | 编码/资产码 | 场外视图 | 不含耗材 | 默认列 |
| 物资名称 | 名称 | — | — | 默认列 |
| 使用地点 | 地点 | `usage_location` | — | 默认列 |
| 数量 | 数量 | — | — | 默认列 |
| 责任人 | 责任人 | 借用人/保管人 | — | 默认列 |
...
| 类型 | 控件 | 对应参数字段 | 说明 |
|------|------|--------------|------|
| 关键字 | 搜索框 | `keyword` | 物资编码、物资名称、使用地点 |

**功能权限（操作项）**

| 操作项 | 权限码 | 可见角色 | 可见条件(状态) | 目标 |
|--------|--------|----------|----------------|------|
| 查看 | `wms:planning:outside:view` | 管理员/仓管/计划 | 全部 | 资产/物资详情 |
...
与 `purchase_plan_apply` **同源接口**；字段同 §17.5.4–18.5.5。权限：`wms:purchase:plan:create`。
...
**弹窗说明**：待采行「查看计划」；展示计划头+明细，不跳转计划列表。尺寸 XL。参数：`planNo` 或计划相关 query、`back`。

| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 必填 | 校验/提交逻辑 |
|--------|------|----------|---------------|------|---------------|
| 计划单号 | JH/JJJH | `material_plan.no` | 横幅 | — | 无提交 |
| 计划类型 | 一般/急件 | `material_plan.plan_type` | 急件→待采 | — | 无提交 |
| 审批状态 | 计划状态 | `material_plan.status` | — | — | 无提交 |
| 计划名称/填报人/部门/申请日期/最早需求日/需求说明 | 计划头 | `material_plan` | — | — | 无提交 |
...
|--------|------|----------|---------------|------|---------------|
| 序号 | 行号 | 前端 | 1..n | — | 无提交 |
| 物资编码/名称/规格/大类/子类/单位 | 清单 | `material_plan_line` | — | — | 无提交 |
| 需求数量 | 计划 qty | `material_plan_line.qty` | — | — | 无提交 |

| 操作项 | 权限码 | 说明 |
|--------|--------|------|
| 关闭 | `wms:purchase:pending:view` | 回待采列表 |
| 跳转物资计划列表/待采物资 | 各 view | 快捷外链 |
...
| 字段名 | 含义 | 数据来源 | 产生/计算规则 | 必填 | 校验/提交逻辑 |
|--------|------|----------|---------------|------|---------------|
| 供货单号/物资/供应商/需求数量等 | 供货头只读 | `supply_order` | 网格展示，见 §17.5.7 | — | 无提交 |
...
**提交逻辑**：`POST /api/v1/scrap-orders/{scrapKey}/execute`；状态=待执行；扣减货位 qty 或 asset→已作废；写 scrap 流水与 audit_log。
...
以下页面以 Modal 承载完整表单，**字段清单与提交校验已在对应 §17 小节**，此处仅索引：

| 弹窗页面 | 说明 | 字段明细 |
|----------|------|----------|
| `apply_plan_form` | 物资计划新增/编辑 | §17.4.2 |
| `apply_requisition_form` | 领用申请 | §17.4.4 |
| `purchase_request_form` | 急件采购申请 | §17.5.3 |
| `purchase_plan_direct` / `inquiry_bid` | 计划采购 | §17.5.5 |
...
> **文档维护**：原型迭代时同步更新 `generate-pages.mjs` 与本章；新增列表页须补齐三张表，新增表单须补齐字段清单表，**新增弹窗须在本章或 §17.13.9 索引登记**。
