...
const WMS_APPLY_PLAN_SAMPLES = {
  ...
  JJJH202510001: {
    no: 'JJJH202510001', name: '设备配件补库', planType: '一般计划', status: '审核通过',
  ...
  JJJH202510002: {
    no: 'JJJH202510002', name: '设备配件补库', planType: '一般计划', status: '审核通过',
  ...
  JJJH202606050001: {
    no: 'JJJH202606050001', name: '维保耗材采购', planType: '一般计划', status: '审核通过',
    ...
    <aside class="glass-sidebar flex w-64 shrink-0 flex-col border-r border-slate-200/80">
      <div class="flex h-16 items-center gap-3 border-b border-slate-200/80 px-5">
        ...
        <div>
          ...
          <div class="text-xs text-slate-500">黄冈武穴 · V1.0.1</div>
        </div>
      </div>
...
function bindQuoteSelectedSelect(sel, table) {
  if (!sel || sel.dataset.wmsQuoteBound === '1') return;
  sel.dataset.wmsQuoteBound = '1';
  sel.addEventListener('change', () => {
    if (sel.value !== 'yes') return;
    table.querySelectorAll('[data-wms-quote-selected]').forEach(other => {
...
function initQuoteSelectedLogic(table) {
  table.querySelectorAll('[data-wms-quote-selected]').forEach(sel => bindQuoteSelectedSelect(sel, table));
}

function renumberQuoteSupplierRows(tbody) {
  [...tbody.querySelectorAll('[data-wms-quote-supplier-row]')].forEach((row, i) => {
    const seq = row.querySelector('.wms-quote-supplier-seq');
    if (seq) seq.textContent = String(i + 1);
  });
}

function syncQuoteSupplierAddState(table) {
  const max = Number(table.dataset.wmsQuoteMaxSuppliers || 1);
  const tbody = table.querySelector('[data-wms-quote-supplier-tbody]');
  const wrap = table.closest('[data-wms-execute-nested-row]') || table.parentElement;
  const addBtn = wrap?.querySelector('[data-wms-quote-supplier-add]');
  const count = tbody?.querySelectorAll('[data-wms-quote-supplier-row]').length || 0;
  const canAdd = max > 1 && count < max;
  if (addBtn) {
    addBtn.classList.toggle('hidden', max <= 1);
    addBtn.disabled = !canAdd;
    addBtn.classList.toggle('opacity-40', !canAdd);
    addBtn.classList.toggle('pointer-events-none', !canAdd);
    addBtn.title = canAdd ? '' : `最多 ${max} 个供应商`;
  }
  const deleteBtns = tbody?.querySelectorAll('[data-wms-quote-supplier-delete]') || [];
  deleteBtns.forEach(btn => {
    const hide = max <= 1 || count <= 1;
    btn.classList.toggle('hidden', hide);
  });
  table.querySelectorAll('[data-wms-quote-op-col], [data-wms-quote-op-cell]').forEach(el => {
    el.classList.toggle('hidden', max <= 1);
  });
}

function buildQuoteSupplierSelectHtml() {
  const options = WMS_EXECUTE_SUPPLIERS.map(
    name => `<li class="wms-search-select-option cursor-pointer px-3 py-2 text-sm text-slate-700 hover:bg-slate-50" data-value="${name}" role="option">${name}</li>`
  ).join('');
...
function createQuoteSupplierRow(isGeneral) {
  const tr = document.createElement('tr');
  tr.className = 'border-t border-slate-100';
  tr.setAttribute('data-wms-quote-supplier-row', '');
  const selectedHtml = isGeneral
    ? `<select data-wms-quote-selected class="min-w-[120px] rounded border border-slate-200 px-2 py-1.5 text-sm outline-none focus:border-slate-400 focus:ring-1 focus:ring-slate-200">
  ...
  const inputCell = '<td class="px-3 py-2 whitespace-nowrap"><input type="text" placeholder="请输入" class="w-full min-w-[72px] rounded border border-slate-200 px-2 py-1.5 text-sm outline-none focus:border-slate-400 focus:ring-1 focus:ring-slate-200" /></td>';
  tr.innerHTML = `
    <td class="px-3 py-2 text-sm text-slate-600 whitespace-nowrap wms-quote-supplier-seq">1</td>
    <td class="px-3 py-2 whitespace-nowrap">${buildQuoteSupplierSelectHtml()}</td>
    ...
    <td class="px-3 py-2 whitespace-nowrap">${selectedHtml}</td>
    ${inputCell}
    <td class="px-3 py-2 text-right text-sm whitespace-nowrap" data-wms-quote-op-cell><button type="button" class="text-rose-600 hover:underline" data-wms-quote-supplier-delete>删除</button></td>
  `;
  return tr;
}

function applyGeneralQuoteSelectedOptions(select) {
  if (!select) return;
  select.innerHTML = `
    <option value="" disabled>请选择</option>
    <option value="yes">已中选</option>
    <option value="no" selected>未中选</option>
  `;
}

function initV101DirectPurchaseMode(form) {
  if (!document.body.hasAttribute('data-wms-v101-direct')) return false;
  const params = new URLSearchParams(location.search);
  const isGeneral = params.get('planType') === 'general';
  const tip = document.querySelector('[data-wms-v101-direct-tip]');
  const setField = (key, value) => {
    const el = form.querySelector(`[data-wms-direct-field="${key}"]`);
    if (el) el.textContent = value;
  };

  if (isGeneral) {
    if (tip) {
      tip.className = 'mb-4 rounded-xl bg-sky-50 px-4 py-3 text-sm text-sky-900 ring-1 ring-sky-200';
      tip.innerHTML = '<strong>一般计划采购</strong>：可添加多个供应商（最多 <strong>5</strong> 个）；同一物资须且仅能有 <strong>1</strong> 个「已中选」；提交后需走物资采购审核，通过后进入供货。';
    }
    setField('poNo', params.get('po') || 'WZCG202606090002');
    setField('planType', '一般计划');
    setField('planNo', 'JH202606090001');
    setField('applyNo', 'JJSQ202606090008');
    setField('amount', '15,800.00');
    form.querySelectorAll('.wms-execute-quote-nested').forEach(table => {
      table.dataset.wmsQuoteMaxSuppliers = '5';
      table.querySelectorAll('[data-wms-quote-selected]').forEach((sel, idx) => {
        applyGeneralQuoteSelectedOptions(sel);
        if (idx === 0) sel.value = 'yes';
      });
      const tipEl = table.parentElement?.querySelector('[data-wms-quote-supplier-tip]');
      if (tipEl) tipEl.innerHTML = '一般计划可添加最多 <strong>5</strong> 个供应商；须且仅能有 <strong>1</strong> 个「已中选」。';
      syncQuoteSupplierAddState(table);
    });
  } else {
    if (tip) {
      tip.className = 'mb-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-900 ring-1 ring-amber-200';
      tip.innerHTML = '<strong>急件直采约束</strong>：采购方式锁定直采；供应商<strong>有且仅有 1 个</strong>；提交后需走物资采购审核，通过后进入供货。';
    }
    if (params.get('po')) setField('poNo', params.get('po'));
    if (params.get('planNo')) setField('planNo', params.get('planNo'));
    setField('planType', '急件计划');
    setField('applyNo', '—');
    form.querySelectorAll('.wms-execute-quote-nested').forEach(table => {
      table.dataset.wmsQuoteMaxSuppliers = '1';
      const tipEl = table.parentElement?.querySelector('[data-wms-quote-supplier-tip]');
      if (tipEl) tipEl.innerHTML = '本期急件直采仅允许填写 <strong>1</strong> 个供应商，不可再增行。';
      syncQuoteSupplierAddState(table);
    });
  }
  return isGeneral;
...
function initQuoteSupplierRowActions(form, isGeneral) {
  form.querySelectorAll('.wms-execute-quote-nested').forEach(table => {
    syncQuoteSupplierAddState(table);
    const wrap = table.parentElement;
    wrap?.querySelector('[data-wms-quote-supplier-add]')?.addEventListener('click', () => {
      const max = Number(table.dataset.wmsQuoteMaxSuppliers || 1);
      const tbody = table.querySelector('[data-wms-quote-supplier-tbody]');
      if (!tbody) return;
      const count = tbody.querySelectorAll('[data-wms-quote-supplier-row]').length;
      if (count >= max) {
        showMaterialToast(`同一物资最多添加 ${max} 个供应商`);
        return;
      }
      const row = createQuoteSupplierRow(isGeneral || max > 1);
      tbody.appendChild(row);
      row.querySelectorAll('[data-wms-supplier-select]').forEach(initSearchableSupplierSelect);
      const sel = row.querySelector('[data-wms-quote-selected]');
      bindQuoteSelectedSelect(sel, table);
      renumberQuoteSupplierRows(tbody);
      refreshSupplierSelectOptions(table);
      syncQuoteSupplierAddState(table);
    });
  });

  form.addEventListener('click', (e) => {
    const del = e.target.closest('[data-wms-quote-supplier-delete]');
    if (!del) return;
    const row = del.closest('[data-wms-quote-supplier-row]');
    const table = del.closest('.wms-execute-quote-nested');
    const tbody = table?.querySelector('[data-wms-quote-supplier-tbody]');
    if (!row || !tbody || !table) return;
    const max = Number(table.dataset.wmsQuoteMaxSuppliers || 1);
    const count = tbody.querySelectorAll('[data-wms-quote-supplier-row]').length;
    if (max <= 1 || count <= 1) {
      showMaterialToast('至少保留 1 个供应商');
      return;
    }
    row.remove();
    renumberQuoteSupplierRows(tbody);
...
function initPurchaseExecuteQuoteForm(root) {
  ...
  if (!form) return;
  const isGeneral = initV101DirectPurchaseMode(form);
  ...
  initQuoteSupplierRowActions(form, isGeneral);
}
